...
CASES.push(
  ...
  },
  {
    ...
    expect: {
      // 17.1-01 Task 3 (G-1512, TRAV-15, decision D-02) folded a bare
      // `skips.counts().oversized > 0` into `incomplete`, exiting 2 here.
      // D-02a then scoped that to NAMED CANDIDATES only (still 2 for this
      // case, marker-config is a candidate class) and measured as a
      // structural no-op on a real tree. D-02b (17.1-CONTEXT.md, FINAL,
      // 2026-08-10) retired BOTH: `oversized` is a SCOPE reason and no
      // longer affects the exit code under any scoping rule -- see
      // lib/traverse/engine.js's ANOMALY_SKIP_REASONS/SCOPE_SKIP_REASONS
      // doc comment for the full rationale, including why G-1512's
      // underlying premise (a poisoned lockfile hiding past the cap) was
      // disproved by test. A marker string past the bulk-content cap is
      // still never READ (no false positive -- pinned by findingCount: 0
      // and the mustNotMatch FAIL guard below, unchanged); the scan is
      // simply no longer marked incomplete solely because of it. This
      // restores the PRE-17.1-01 status: 0 expectation.
      status: 0,
      findingCount: 0,
      mustNotMatch: [/\[FAIL\]/],
    },
    note: `Pins the bulk-content size bound (${BULK_CAP} bytes, script's \`-size -256k\`): a marker string past the cap is never read, so it does not FAIL. This is the bound D-24's hash-candidate tier is deliberately EXEMPT from (see manifests/waves/chaindrop-aug2026.json knownBadHashes[0].description) — the marker-string bulk scan and the hash-candidate scan use different caps on purpose. As of 17.1-01 D-02b (FINAL), a skipped-oversized file is a disclosed SCOPE boundary (\`[skip] oversized: N\`), not an ANOMALY -- it no longer flips the exit code.`,
  },
  {
    id: 'bun-staging',
