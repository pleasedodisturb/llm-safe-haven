...
function findEnvFilesDetailed(startDir, maxDepth) {
  ...
  // ANOMALY_SKIP_REASONS member (today {unreadable, budget}; plan 18-05
  // adds `swapped`), not just 'unreadable' -- a budget-truncated or
  ...
  // term is one `stopped` already flags. This also means plan 18-05's
  // `swapped` reason is picked up automatically the moment it lands, though
  // a read pool is never constructed on this enumerateSync() path, so it
  // can never actually fire here.
...
function scanForEnvFilesDetailed() {
  ...
  // rootFailures: { missing, unreadable } -- `missing` counts every
  // occurrence getRoots() dropped via onMissingRoot (D-07b's sibling
  // seam, ENOENT/ENOTDIR); `unreadable` is filled by plan 18-04 Task 3's
  // onUnreadableRoot seam (EXIT-02, D-07b) -- 0 here until that lands.
  return {
    files: [...new Set(allFiles)].sort(),
    incomplete,
    anomalyCount,
    anomalyReasons,
    rootFailures: { missing: missingRoots.length, unreadable: 0 },
  };
}
...
function scan(flags, opts) {
  ...
  // G-1511 / TRAV-14, extended by G-1545/D-07a: the exit-code contract
  // (0 = clean, 1 = findings, 2 = error-or-incomplete) requires this path
  // to never exit 0 when the scan did not finish. `settleCommand`
  // (lib/cli.js) already propagates a numeric `result.code` SYNCHRONOUSLY
  // for a plain-object return -- read directly, this needs no lib/cli.js
  // change.
  //
  // D-07a (2026-08-12, this plan): unreadable directories encountered
  // during this enumeration NOW fold into `incomplete` via
  // scanForEnvFilesDetailed()'s anomalyCount (findEnvFilesDetailed, Part
  // A above). This REVERSES the prior deferral recorded here (`git log`
  // this line's history): a `.env` behind a mode-000 directory used to
  // print a GREEN CHECK and exit 0 -- reproduced three times on b25b8b5.
  // The "far wider blast radius" concern that justified the deferral was
  // MEASURED, not assumed: `unreadable: 0` across the real default root
  // set on the reference machine, because this enumeration's own
  // maxDepth:4 + SKIP_DIRS + skipDotDirs walk is narrow by construction.
  // This phase IS the separate product decision the old comment deferred
  // -- the justification is the internal inconsistency it left standing:
  // the same `.env` already made `audit` exit 1, `scan --mcp` exit 1, and
  // `scan --supply-chain` exit 1, while this path alone exited 0.
  //
  ...
  // stdout and stderr can never name different causes for one run.
  if (incomplete) {
    const clauseText = buildCauseClauses(detail).map((c) => c.text).join('; ');
    console.error(`llm-safe-haven: the secret scan did not finish (${clauseText}) \u2014 results are incomplete; exiting 2 rather than reporting clean`);
    return { code: 2 };
  }
}

module.exports = { scan, scanForEnvFiles, scanForEnvFilesDetailed, findEnvFiles, findEnvFilesDetailed, printEnvScanResult, runSupplyChainScan, SUPPLY_CHAIN_SCANNER };
