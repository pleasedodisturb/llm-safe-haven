 ...
 *
 * D-20-01 / EXIT-04 (G-1621), additive: `opts.cwd` (default
 * `process.cwd()`) is the injectable seam that makes the zero-default-root
 * cwd fallback testable without ever scanning this repo's own working tree
 * (research Pitfall 1) -- matching the `opts.homedir`/`opts.env`/`opts.fs`
 * DI convention lib/roots.js already uses. When `getRoots()`'s NEW
 * `onNoDefaultRoots` seam fires AND no default candidate was merely
 * unreadable (`unreadableRoots.length === 0` -- an unreadable-root run is
 * already incomplete for a STATED reason, and claiming "no default scan
 * root found" on top of that would be factually wrong), the run either (a)
 * scans `opts.cwd` alone, when it looks like a project, printing exactly
 * one stderr line, or (b) is `incomplete = true` with `rootResolution.
 * unresolved = true` -- reaching `computeExit()`'s existing precedence via
 * the SAME `incomplete` boolean every other anomaly already uses, never a
 * parallel exit path (D-04's anti-pattern).
 */
function scanForEnvFilesDetailed(opts = {}) {
  const { getRoots, resolveZeroRootFallback, CWD_FALLBACK_NOTICE } = require('./roots.js');
  ...
  const roots = getRoots({
    ...
    // D-20-05 (G-1621): fires once, post-loop, only when the DEFAULT probe
    // (not explicit LSH_ROOTS) resolved to zero roots -- see its own doc
    // comment in lib/roots.js.
    onNoDefaultRoots: () => { noDefaultRootsFired = true; },
  });

  let effectiveRoots = roots;
  let rootResolution = { unresolved: false, cwdFallback: null };

  if (noDefaultRootsFired && unreadableRoots.length === 0) {
    const fallbackRoot = resolveZeroRootFallback({ cwd });
    if (fallbackRoot) {
      effectiveRoots = [fallbackRoot];
      console.error(`llm-safe-haven: ${CWD_FALLBACK_NOTICE}`);
      rootResolution = { unresolved: false, cwdFallback: fallbackRoot };
    } else {
      incomplete = true;
  ...
  for (const dir of effectiveRoots) {
  ...
  //
  // rootResolution (G-1621, additive): `unresolved` is true ONLY in the
  // exit-2 branch above (zero default roots AND no project-looking cwd);
  // `cwdFallback` carries the resolved directory when the cwd fallback
  // fired instead. Every existing field above keeps its name, type and
  // position unchanged.
  return {
...
// other consumers.
function scanForEnvFiles(opts) {
  return scanForEnvFilesDetailed(opts).files;
}
 ...
 * no-root, budget, unreadable, swapped, root -- callers that need "the"
 * cause (the next-steps remedy) take clauses[0].
 *
 ...
 *
 * D-20-01(2) / EXIT-04 (G-1621): `rootResolution` defaults to the
 * NON-FIRING shape (`{ unresolved: false, cwdFallback: null }`) --
 * `buildCauseClauses()` explains an already-decided verdict, it does not
 * decide one, and the G-1617 drift test below feeds it hand-built details
 * with no `rootResolution` field at all; the default MUST be the safe one
 * or every one of those cases would gain a spurious second clause. Placed
 * FIRST (not appended) so `clauses[0]` -- which drives the next-steps
 * remedy -- names the real cause. A NEW clause id (`no-root`), not the
 * existing `root` id: the two fire from DISJOINT conditions (`rootFailures`
 * is populated only via onMissingRoot/onUnreadableRoot, which never fire
 * for a default-probe absence per D-17.1-B), and conflating their ids would
 * print the wrong remedy for one of the two.
 */
function buildCauseClauses(detail) {
  const rootResolution = detail.rootResolution || { unresolved: false, cwdFallback: null };
  ...
  if (rootResolution.unresolved) {
    const { NO_SCAN_ROOT_CAUSE } = require('./roots.js');
    clauses.push({ id: 'no-root', text: NO_SCAN_ROOT_CAUSE });
  }
  if (reasons.budget > 0) {
    clauses.push({
      id: 'budget',
...
function remedyForCause(clauseId, command, C) {
  ...
  switch (clauseId) {
    case 'no-root':
      return `Set LSH_ROOTS to the directory you want scanned, or re-run ${cmdText} from a project directory`;
    case 'budget':
...
function scan(flags, opts) {
  ...
  // D-20-01 / EXIT-04 (G-1621): threads opts.cwd through to the
  // zero-default-root fallback decision. `scanForEnvFilesDetailed`
  // defaults to `process.cwd()` when this is omitted/undefined.
  const detail = scanForEnvFilesDetailed({ cwd: opts && opts.cwd });
