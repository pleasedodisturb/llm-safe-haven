...
const { getMcpInputs, normalizeAuditResult } = require('./audit.js');
...
async function install(flags) {
  ...
  for (const agent of found) {
    ...
    // WR-03 boundary normalization (shared with lib/audit.js): a
    // non-throwing but malformed audit() return must never poison the
    // level math or crash the renderer — a broken module never crashes
    // the CLI (CLAUDE.md).
