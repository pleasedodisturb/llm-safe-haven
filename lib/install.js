...
const { getMcpInputs } = require('./audit.js');
...
async function install(flags) {
  ...
  for (const agent of found) {
    ...
    // WR-03-style boundary normalization: a non-throwing but malformed
    // audit() return (null, or a non-numeric level) must never poison
    // the level math (Math.max over undefined -> NaN) or crash the
    // renderer — a broken module never crashes the CLI (CLAUDE.md).
    if (!auditResult || typeof auditResult.level !== 'number') {
      auditResult = { checks: [], level: 0 };
    } else if (!Array.isArray(auditResult.checks)) {
      auditResult = { ...auditResult, checks: [] };
    }
  ...
  // 4. Scan for .env files + offline MCP scan (CR-01: same inputs, same
  // unified level function as `audit` — a verified MCP finding or an
  // incomplete scan demotes the printed level exactly like tracked .env
  // files do; getMcpInputs fails closed to mcp-incomplete on any throw).
