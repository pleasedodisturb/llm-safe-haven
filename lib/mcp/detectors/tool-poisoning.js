...
function derivePackageName(args) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    // Not a well-formed npm package spec (e.g. contains a path separator,
    // like ../../evil) — refuse to derive a name rather than falling back
...
/**
 * Joins pkgName's segments under `root` and verifies — on the RESOLVED
 * path — that the result is still contained within `root`. Returns the
 * package.json path, or null when containment fails. Belt-and-braces on
 * top of isSafePackageName(): no input shape may ever produce a read
 * outside the node_modules / npx-cache root (T-05-09 boundary).
 */
function containedPackageJsonPath(root, pkgName) {
  const resolvedRoot = path.resolve(root);
  const joined = path.resolve(resolvedRoot, ...pkgName.split('/'), 'package.json');
  if (!joined.startsWith(resolvedRoot + path.sep)) return null;
  return joined;
}
...
function resolveLocalPackageDescription(pkgName, opts = {}) {
  // CR-02: refuse anything that is not a plausible npm package name
  // BEFORE any path join — attacker-controlled args must never drive a
  // filesystem read outside node_modules / the npx cache (T-05-09).
  if (!pkgName || !isSafePackageName(pkgName)) return null;
  const cwd = opts.cwd || process.cwd();
  const homedir = opts.homedir || os.homedir();

  // 1. Project-local node_modules — the common, fast case.
  const localPkgJson = containedPackageJsonPath(path.join(cwd, 'node_modules'), pkgName);
  if (localPkgJson !== null) {
  ...
  for (const hash of hashDirs) {
    const candidatePkgJson = containedPackageJsonPath(path.join(npxDir, hash, 'node_modules'), pkgName);
    if (candidatePkgJson === null) continue;
