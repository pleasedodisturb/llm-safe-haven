 ...
 * detector in the codebase. index.js's runAll() wiring to await async
 * detectors is Phase 7's job, not this phase's; the { id, requirement,
 * run } shape is unchanged, only run()'s return type widens to a
 * Promise (compatible with index.js's "only id and run are load-
 * bearing" contract).
 */
