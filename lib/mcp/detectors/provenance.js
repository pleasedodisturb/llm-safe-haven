...
async function fetchAttestationStatus(pkgName, versionOrTag, fetchImpl) {
  ...
  try {
    response = await fetchImpl(buildRegistryUrl(pkgName, versionOrTag), {
      ...
      // SSRF hardening: the request host is a hardcoded literal
      // (REGISTRY_HOST), but a compromised/MITM'd registry could reply
      // 3xx + Location to pivot the scanner onto an attacker-chosen host
      // (incl. link-local 169.254.169.254). The project invariant is
      // "never fetch attacker-controlled URLs" — so a redirect away from
      // the pinned host is a lookup failure, never followed.
      redirect: 'error',
    });
  } catch {
