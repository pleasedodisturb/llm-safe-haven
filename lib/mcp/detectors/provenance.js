...
const { commandBasename, derivePackageName, extractSpec, versionFromSpec, isSafePackageName } = require('../npx-args.js');
...
// WR-02 suffix classification (mirrors unpinned-execution.js's
// EXACT_SEMVER_RE discipline). An exact semver — optionally v-prefixed;
// the registry rejects the v, so it is normalized away via the capture
// group — is the ONLY pinned shape. A range/wildcard/partial spec
// (^ ~ > < = operators, |, whitespace, *, x-ranges, bare/partial
// numerics like 1 or 1.2) is not a registry-fetchable path segment;
// fetching it raw produced a noisy fetch-failed instead of a real
// answer against latest. Anything left is a dist-tag (next, beta, ...):
// directly fetchable, but still a floating ref.
 ...
 * (npx-args.js, WR-06 consolidation: the flag-scan loop lives there,
 * never re-copied here). Returns { ref, pinned }:
 *   pinned spec (name@1.2.3, name@v1.2.3) -> { ref:'1.2.3', pinned:true }
...
function deriveVersionOrTag(args) {
  ...
  if (RANGEISH_RE.test(suffix)) return { ref: 'latest', pinned: false };
  return { ref: suffix, pinned: false };
}
