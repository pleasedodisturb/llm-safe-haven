...
const { commandBasename, extractSpec, versionFromSpec, classifySpecSuffix, gitSpecPin } = require('../npx-args.js');
...
// (WR-06 consolidation) — the npx '--package'/'-p' value, or the uvx
// '--from' value, is the package that gets installed; the trailing
// positional token is just the binary name to execute, which never
// carries a version, so version-checking it produced false 'unpinned'
// findings on correctly-pinned `npx -p pkg@1.0.0 serve` invocations
// (WR-03). Deliberate semantic unifications: a spec-designating flag
// with a NON-STRING or flag-shaped value refuses to derive (null → no
// spec → no finding) instead of falling through to the trailing command
// token — the shared refusing semantics are documented in npx-args.js
// (F5). uvx's --from handling moved into extractSpec itself (F1), which
// also skips known value-taking flags (uvx -p/--python, npx --loglevel,
// ...) whose values previously masqueraded as the positional spec.
...
function run(servers, context = {}) {
  ...
  for (const server of Array.isArray(servers) ? servers : []) {
    ...
    if (bin === 'npx' || bin === 'uvx') {
      ...
      const spec = extractSpec(args, bin);
      // G-1368: git direct-reference specs are classified by gitSpecPin()
      // BEFORE the SPEC_RE pipeline — SPEC_RE cannot parse a git URL, so
      // isPinnedPackageArg saw null and flagged every git spec, even one
      // pinned to an immutable full commit SHA. Pinned = @<40-hex SHA>
      // only; a missing ref, branch, tag, or short SHA stays unpinned.
      const gitPin = gitSpecPin(spec);
      const unpinned = spec !== null &&
        (gitPin !== null ? gitPin !== 'exact' : !isPinnedPackageArg(spec));

      if (unpinned) {
        // The raw spec is NEVER interpolated into the message — a git URL
        // can carry credentials in its authority (CR-01 invariant).
        findings.push(Finding({
          ...
          message: gitPin !== null
            ? `Server "${server.name}" runs ${bin} from a git URL without an immutable commit-SHA ref — a branch or tag can silently move to a compromised commit.`
            : `Server "${server.name}" runs ${bin} without a pinned package version — an unpinned spec can silently pull a compromised update.`,
        }));
      }
    }
