 ...
 * still apply to it.
 *
 * G-1376: a full-spec entry's name half only seeds the WR-04 exact-name
 * pool if it is NOT composed entirely of generic tokens (mcp, server).
 * "@sentry/mcp-server" legitimately seeds the generic half "mcp-server"
 * into fullSpecNameHalves — but many vendors independently and
 * legitimately publish "@vendor/mcp-server" (ui5, cap-js, launchdarkly,
 * hubspot, browserstack — 5 confirmed FPs from the G-1372 top-100 scan),
 * and the exact-name heuristic otherwise treats every one of them as
 * "the Sentry package under an unrecognized scope". A separate derived
 * pool, scopeConfusionNameHalves (fullSpecNameHalves minus fully-generic
 * halves), is checked here INSTEAD OF fullSpecNameHalves; the near-match
 * classes above still use nameCandidates (which is NOT filtered), so
 * e.g. "mcp-servr" keeps firing near-known-name against "mcp-server".
 * Token-composition, not a stoplist: a half with ANY token outside the
 * generic set (e.g. "context7-mcp", "notion-mcp-server") stays eligible
 * for scope-confusion.
 *
...
// G-1376: name halves composed ENTIRELY of generic tokens (mcp, server)
// are too weakly distinctive to seed the WR-04 scope-confusion exact-name
// pool -- many vendors independently publish "@vendor/mcp-server" and the
// exact-match heuristic otherwise treats every one as "the seeding
// vendor's package under the wrong scope". Token-composition, not a
// stoplist: a half with ANY token outside this set stays eligible.
const GENERIC_NAME_TOKENS = new Set(['mcp', 'server']);
function isGenericNameHalf(half) {
  return half.split('-').every(token => GENERIC_NAME_TOKENS.has(token));
}
...
function indexAllowlist(manifest) {
  ...
  // weaker scan).
  //
  // WR-01 (G-1376): a FULLY-GENERIC scopedOnly[] entry is rejected the
  // same way. scopedOnly names are deliberately EXEMPT from the
  // genericness bar at the scope-confusion check site (D-04 — they are
  // hand-curated manifest data), so a generic entry like "mcp-server"
  // would silently reintroduce the exact FP class G-1376 kills, with
  // nothing else catching it. That is a malformed manifest — the same
  // class as scopedOnly/servers drift above — so it fails closed
  // (allowlist-unavailable) rather than weakening the invariant.
  for (const name of scopedOnlyNames) {
    if (!bareServers.has(name)) return { ok: false };
    if (isGenericNameHalf(name)) return { ok: false };
  }
  ...
  // G-1376: WR-04-only filtered pool -- nameCandidates (class-1) keeps
  // ALL full-spec name halves (including generic ones) so e.g.
  // "mcp-servr" still near-matches "mcp-server"; only the scope-confusion
  // EXACT-match check below excludes generic halves.
  const scopeConfusionNameHalves = new Set();
  for (const half of fullSpecNameHalves.keys()) {
    if (!isGenericNameHalf(half)) scopeConfusionNameHalves.add(half);
  }

  return { ok: true, knownScopes, scopedOnlyNames, bareServers, fullSpecServers, fullSpecNameHalves, nameCandidates, scopeConfusionNameHalves };
}
...
function run(servers, context = {}) {
  ...
  const { knownScopes, scopedOnlyNames, bareServers, fullSpecServers, fullSpecNameHalves, nameCandidates, scopeConfusionNameHalves } = index;
  ...
  for (const server of Array.isArray(servers) ? servers : []) {
    ...
    if (pkgNameOnly.length >= 4 && (scopedOnlyNames.has(pkgNameOnly) || scopeConfusionNameHalves.has(pkgNameOnly))) {
