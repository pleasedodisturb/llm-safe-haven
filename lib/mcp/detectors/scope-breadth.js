...
/**
 * Extracts package identifier tokens from an npx/uvx args array. The
 * capability-bearing package name can appear EITHER as the positional
 * spec OR as the value of `-p`/`--package`/`--from` — e.g.
 * `npx -p mcp-shell-server run` runs mcp-shell-server, whose name lives
 * only in the flag value, not the positional token. All such flag values
 * are genuine package identifiers (not free-form shell text, so the D-12
 * "identifier tokens only" invariant still holds) and are collected as
 * tokens, plus the first positional token. Both the split (`-p spec`) and
 * joined (`--package=spec`) forms are handled, mirroring
 * unpinned-execution's extractPackageSpec.
 */
function extractPackageTokens(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const joined = arg.match(/^(?:--package|--from)=(.+)$/);
    if (joined) { tokens.push(joined[1]); continue; }
    if (arg.startsWith('-')) continue;
    tokens.push(arg);
    break; // stop after the first positional token
  }
  return tokens;
}
