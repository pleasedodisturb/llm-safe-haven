...
/**
 * WR-04 gate for sensitive-name-literal: the key NAME matching
 * SENSITIVE_KEY_RE is not enough on its own — common non-secret config
 * (AUTH_TYPE=oauth, CREDENTIAL_PROCESS=aws-vault, TOKEN_ENDPOINT=https://…,
 * PASSWORD_MIN_LENGTH=12) would otherwise fire HIGH false positives.
 * The VALUE must also plausibly be a secret: at least 12 chars, no
 * whitespace, and not an obviously structural value (boolean, pure
 * integer, or URL-shaped).
 */
function looksSecretLike(value) {
  ...
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(value)) return false; // URL-shaped
  return true;
}
...
function run(servers, context = {}) {
  ...
  for (const server of Array.isArray(servers) ? servers : []) {
    ...
    for (const [key, value] of Object.entries(env)) {
      ...
      // WR-04 ordering: a wildcard/whole-env passthrough token is a
      // scoping smell (LOW), even when it sits under a sensitive-sounding
      // key name — classify it as broad-inheritance BEFORE the
      // sensitive-name branch can claim it as a HIGH literal.
      ...
      if (SENSITIVE_KEY_RE.test(key) && looksSecretLike(value)) {
        findings.push(Finding({
