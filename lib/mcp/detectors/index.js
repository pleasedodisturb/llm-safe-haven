 ...
 *
 * async since Phase 6 (CR-01): provenance.js's run() returns a Promise,
 * and a sync loop structurally CANNOT contain a rejected promise — the
 * rejection escaped the try/catch as an unhandled rejection (process
 * crash under Node's default policy) and the Promise result failed the
 * Array.isArray gate, silently dropping the detector's findings. The
 * `await` handles sync and async detectors uniformly: a sync return
 * value passes through unchanged, a rejected promise becomes a caught
 * throw. Returns Promise<Finding[]>; Phase 7's orchestrator wiring
 * inherits this contract.
 */
async function runAll(servers, context = {}) {
  const detectors = loadDetectors();
  const findings = [];
  for (const detector of detectors) {
    try {
      const result = await detector.run(servers, context);
      if (Array.isArray(result)) findings.push(...result);
    } catch {
      // A throwing OR rejecting detector must never crash the whole
      // scan (D-01) — the await above turns an async rejection into a
      // synchronous throw this catch can contain.
    }
  }
  return findings;
}
