...
// A git direct-reference spec (pip/uvx VCS grammar: git+https://...,
// git+ssh://..., bare git://...). SPEC_RE cannot parse these — the URL's
// slashes break the name grammar, so splitSpec refuses (null) and every
// downstream helper returns null; git specs need their own pin
// classification (G-1368).
const GIT_SPEC_RE = /^git(?:\+|:\/\/)/;
// A full 40-hex commit SHA is the ONLY immutable git ref. Branches and
// tags move; a short SHA (7-39 hex) is prefix-ambiguous, so both stay
// unpinned. Uppercase hex is accepted (the ref is lowercased before this
// check — git resolves SHAs case-insensitively).
const GIT_FULL_SHA_RE = /^[0-9a-f]{40}$/;

/**
 * G-1368: pin classification for git direct-reference specs. SPEC_RE's
 * refusal on URLs made classifySpecSuffix see null, so a git spec pinned
 * to an immutable full commit SHA was still flagged unpinned. Returns:
 *   'exact'    — @<full 40-hex commit SHA> after the URL path (the only
 *                immutable ref)
 *   'unpinned' — no @ref, or a mutable ref: branch, tag, short SHA
 *   null       — not a git spec; caller falls through to the
 *                SPEC_RE-based classification
 * The ref separator is the LAST '@' AFTER the last '/': a git+ssh
 * authority (git+ssh://git@host/org/repo@ref) carries an earlier '@'
 * that is never the ref separator. A pip-style #fragment (#egg=,
 * #subdirectory=) is not part of the ref.
 */
function gitSpecPin(spec) {
  if (typeof spec !== 'string' || !GIT_SPEC_RE.test(spec)) return null;
  const hashIdx = spec.indexOf('#');
  const base = hashIdx === -1 ? spec : spec.slice(0, hashIdx);
  const atIdx = base.lastIndexOf('@');
  if (atIdx <= base.lastIndexOf('/')) return 'unpinned';
  const ref = base.slice(atIdx + 1).toLowerCase();
  return GIT_FULL_SHA_RE.test(ref) ? 'exact' : 'unpinned';
}
