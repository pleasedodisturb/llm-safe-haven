...
/**
 * Windsurf MCP config parser. Windsurf stores its config as JSONC (comments
 * + trailing commas tolerated) at ~/.codeium/windsurf/mcp_config.json
 * (lib/mcp/config-sources.js owns the exact path — this module only reads
 * whatever source.path it's given).
 *
...
function parse(source, opts = {}) {
  ...
  for (const [name, serverConfig] of Object.entries(entries)) {
    ...
    // Windsurf's documented remote-server shape uses `serverUrl`; map it
    // into the normalized `url` slot when `url` itself is absent.
