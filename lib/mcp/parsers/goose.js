...
function parse(source, opts = {}) {
  ...
  let nextIndex;
  try {
    ({ obj: extensionsObj, nextIndex } = parseMappingBlock(lines, firstIdx, blockIndent));
  } catch (err) {
    return { ok: false, reason: 'unparseable', code: 2, detail: err.message };
  }

  // CR-01 (review, Phase 12): parseMappingBlock silently `break`s on the
  // first line at the block indent that isn't a bare-identifier key
  // (`lineIndent !== indent` -> break). checkUnsupportedShape's pre-pass
  // does NOT reject every valid-YAML key shape that fails that
  // bare-identifier grammar -- dotted keys (`foo.bar:`), leading-digit
  // keys (`1key:`), and merge keys (`<<:`) all pass the pre-pass but stop
  // the mapping parse short. Discarding `nextIndex` (as before) means the
  // caller never notices parsing ended early -- an outer break inside a
  // nested extension bubbles up and silently drops every extension that
  // follows the offending line, degrading to `{ ok:true, servers:[] }`
  // while a real Goose YAML loader parses and runs the hidden server.
  // Fail closed instead: walk forward from `nextIndex` and treat any
  // non-blank, non-comment, indented (indent > 0) line before the next
  // legitimate indent-0 sibling key (or EOF) as unparsed residue -- proof
  // the block was NOT fully consumed. This also catches the nested case
  // (a stray key inside one extension's own mapping stops that inner
  // parseMappingBlock short, which in turn stops the OUTER extensions
  // loop short at the same offending indent) because the residue check
  // walks from wherever parsing actually halted, regardless of nesting
  // depth. Same "never silently mis-parse" contract as the rest of this
  // module (MCPC-03).
  for (let k = nextIndex; k < lines.length; k++) {
    if (isBlankOrComment(lines[k])) continue;
    if (indentOf(lines[k]) === 0) break; // legitimate sibling top-level key
    return {
      ok: false,
      reason: 'unparseable',
      code: 2,
      detail: `unparseable line inside extensions block: ${lines[k].trim()}`,
    };
  }
