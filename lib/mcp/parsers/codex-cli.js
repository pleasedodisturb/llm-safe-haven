...
function parse(source, opts = {}) {
  ...
  // WR-02 (review, Phase 12): real TOML treats redefining a table as a
  // fatal error. The prior `if (!servers.has(name))` guard only decided
  // whether to (re)initialize the server's accumulator object -- it did
  // NOT reject a second `[mcp_servers.<name>]` (or a second
  // `[mcp_servers.<name>.env]`) header, so a duplicate table silently
  // last-write-wins on every key it redefines. Track each (name, section)
  // header pair seen so far and fail closed on a repeat, mirroring the
  // D-01 both-env-forms rejection's "never silently pick one" contract.
  // `[mcp_servers.fs]` + `[mcp_servers.fs.env]` for the SAME server is
  // legitimate TOML (two distinct tables in the dotted namespace) and
  // must stay allowed -- only a REPEAT of the identical header is
  // rejected. Two separate Sets (rather than one Set of concatenated
  // strings) sidestep any delimiter-collision risk from quoted table
  // names, which may contain arbitrary characters.
  const seenServerHeaders = new Set();
  const seenEnvHeaders = new Set();
  ...
  for (const rawLine of lines) {
    ...
    if (headerMatch) {
      ...
      const section = headerMatch[2] ? 'env' : 'server';
      const seenHeaders = section === 'env' ? seenEnvHeaders : seenServerHeaders;
      if (seenHeaders.has(name)) {
        const headerText = section === 'env' ? `mcp_servers.${name}.env` : `mcp_servers.${name}`;
        return { ok: false, reason: 'unsupported-toml', code: 2, detail: `duplicate table [${headerText}]` };
      }
