 ...
 *   - symlink (lstatSync pre-check for a clear error message, backed by
 *     O_NOFOLLOW at open time so a symlink swapped in AFTER the lstat is
 *     still refused at the kernel level — no TOCTOU window)
 *   - non-regular file (FIFO/device/socket — a FIFO would block a plain
 *     open/read forever waiting for a writer, a hard DoS; the open uses
 *     O_NONBLOCK so it can never block, and fstat rejects anything that
 *     is not a regular file BEFORE any read)
 *   - oversized (fstat size check BEFORE the read, so the cap is enforced
 *     before the file is loaded into memory)
 *   - unreadable/nonexistent (open/read failure)
 *
 * TOCTOU hardening: the file is opened ONCE and every subsequent guard
 * (fstatSync) and the read itself (readFileSync(fd)) operate on that
 * same fd — a single kernel object, with no path re-resolution between
 * the checks and the read. A racing writer can no longer swap in a
 * symlink after the guards pass.
 *
...
function readConfigSafe(filePath, opts = {}) {
  ...
  const constants = (fs.constants || require('fs').constants);
  ...
  let fd;
  try {
    // O_NONBLOCK: opening a FIFO for reading must never block waiting
    // for a writer. O_NOFOLLOW (where the platform provides it): refuse
    // a symlink at the final path component even if it appeared after
    // the lstat above.
    let flags = constants.O_RDONLY | constants.O_NONBLOCK;
    if (constants.O_NOFOLLOW) flags |= constants.O_NOFOLLOW;
    fd = fs.openSync(filePath, flags);
  } catch (err) {
    if (err.code === 'ELOOP') {
      // O_NOFOLLOW hit a symlink the lstat pre-check missed (raced in).
      return {
        ok: false,
        reason: 'symlink',
        code: 2,
  ...
  try {
    ...
    try { fs.closeSync(fd); } catch { /* fd already closed/invalid */ }
  }
}
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (inString) {
      ...
      if (c === '\\') {
        ...
        // string on it — but only when one exists: a backslash as the
        // very last character (unterminated escape) must not append the
        // literal string "undefined" (text[i+1] === undefined) to
...
function normalizeServer({ agentId, scope, configPath, name, command, args, env, url, headers }) {
  // env/headers are attacker-controlled nested objects: strip prototype-
  // pollution own-keys here at the choke point so the normalized shape
  // GUARANTEES no __proto__/constructor/prototype key ever reaches a
  // Phase 5+ detector — an Object.assign(target, server.env) downstream
  // would otherwise invoke the __proto__ setter.
  return {
    agentId,
    scope,
    configPath,
    ...
    env: (env && typeof env === 'object' && !Array.isArray(env)) ? stripProtoPollution(env) : {},
    url: typeof url === 'string' && url !== '' ? url : null,
    headers: (headers && typeof headers === 'object' && !Array.isArray(headers)) ? stripProtoPollution(headers) : {},
  };
}

/**
 * Splits a findings array into verified/unverified counts — THE single
 * place the confidence-filter expression exists (Phase 8 review F8;
 * previously copy-pasted across lib/audit.js getMcpInputs and both
 * lib/scorecard.js renderers). Additive export, part of no frozen
 * contract. Non-array input counts as zero of each (never throws).
 */
function splitFindingsByConfidence(findings) {
  const list = Array.isArray(findings) ? findings : [];
  const verifiedCount = list.filter(f => f && f.confidence === CONFIDENCE.VERIFIED).length;
  return { verifiedCount, unverifiedCount: list.length - verifiedCount };
}
