...
function scanMcp(flags = {}, opts = {}) {
  // WR-01: the --online flag is parsed (Phase 6, D-11) but the detector
  // loop that consumes it is Phase 7's wiring — until then the flag is
  // an accepted no-op, and an opt-in security check that LOOKS like it
  // ran is a false-clean hazard. Say so loudly on stderr (never stdout —
  // --json output stays pure JSON). The envelope's "offline": true stays
  // accurate: no network call was made.
  if (flags && flags.online === true) {
    console.error(
      'note: --online was accepted, but live registry provenance checks are not yet active in this version — this scan ran fully offline. Online provenance verification activates in the next release.'
    );
  }
