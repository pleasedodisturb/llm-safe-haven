...
function run(argv) {
  ...
  switch (args.command) {
    case 'install': {
      ...
      // same offline getMcpInputs path audit uses). Mirrors the
      // audit/scan thenable pattern below. Exit-code contract preserved:
      // success resolves without touching process.exitCode (implicit 0,
      // as before), the no-agents branch still process.exit(0)s, and a
      // failure — which previously escaped run() as a synchronous throw
      // and crashed bin/ with a non-zero code — now fails with a one-line
      // stderr diagnostic and exitCode 1 (still non-zero, no unhandled
      // rejection).
      const { install } = require('./install.js');
      const result = install(args.flags);
    ...
    case 'audit': {
      // Mirrors the scan-case thenable pattern below (D-01-style): audit()
      // is async as of Phase 8 (D-04/D-05), returning { code } rather than
      // exiting the process itself. Audit exit contract (see
      // auditExitCode() in lib/audit.js): 0 = Level 2+, 1 = Level < 2,
      // 2 = the in-process MCP scan did not complete (fail closed — the
      // locked security-gate-exit-codes rule) or an escaped error below.
      // The defensive sync branch is kept for
      // consistency even though audit() always returns a promise today.
      // A fail-closed .catch guards against an escaped rejection leaving
      // process.exitCode at a stale/default value (T-08-05).
      const { audit } = require('./audit.js');
      const result = audit(args.flags);
      if (result && typeof result.then === 'function') {
        return result
          .then(r => {
          ...
          .catch((err) => {
            // WR-04: exit 2 with zero bytes of explanation is
            // undebuggable for a security gate — someone has to act on a
            // fail-closed result, which requires knowing what happened.
            // One line on stderr; --json stdout purity is preserved.
    ...
    case 'scan': {
      ...
      // Synchronous results assign process.exitCode SYNCHRONOUSLY —
      // wrapping them in Promise.resolve().then() would defer the
      // assignment by a microtask tick, a behavior change vs pre-Phase-7
      // for programmatic callers of the exported run() that read
      // process.exitCode right after it returns. Only a thenable (the
      // --mcp path) goes through the async chain. Assignment to
      ...
      // run() returns the pending promise so embedders/tests can await
      // completion (bin/ ignores the return value).
      if (result && typeof result.then === 'function') {
        return result
          .then(r => {
            if (r && typeof r.code === 'number') process.exitCode = r.code;
