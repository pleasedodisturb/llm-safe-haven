 ...
 *
 * 2026-08-07: there is no longer a gitignore-consulting tier. D-13
 * originally split marker-string scanning into a targeted `marker-config`
 * class (`.env`/`.env.*`/`.npmrc`, never gitignore-pruned) and a bulk
 * `bulk-content` class (every other allow-listed name/extension, pruned by
 * `.gitignore`). Human review of plan 17-14 ruled that split a real
 * detection regression against the pre-retrofit bash scanner (which never
 * consulted gitignore for its marker-string scan at all) -- landing a
 * payload in an already-gitignored path is an attacker-addressable blind
 * spot in a supply-chain scanner, and removing the tier costs nothing
 * measurable (`bulk-content`'s allowlist is narrow; a full `$HOME` scan
 * went from 11,040 ms to 7,862-10,714 ms after the removal). `marker-config`
 * was widened to cover every name `spec.classes['bulk-content'].fileGlobs`
 * lists, `bulk-content` can no longer be assigned by `classify()` at all,
 * and `lib/traverse/git-ignore.js` (the module that supplied the gitignore
 * resolver `bulk-content` was the only consumer of) has been deleted
 * entirely, along with `lib/traverse/engine.js`'s two call sites that
 * constructed it. See `isMarkerConfigMember`'s doc comment below and
 * `docs/wave-spec.md`'s tiering-rules section for the full account.
 */
...
// A1 row 6b, script lines 490-494: PRUNE_COMMON plus node_modules plus
// `.cache` excluded. Since the 2026-08-07 tiering-trade-off reversal (see
// `isMarkerConfigMember` below), this is the ONLY prune scope the
// marker-string scan uses -- there is no longer a separate bulk-tier prune
// scope, because there is no longer a separate bulk tier.
 ...
 * always-run tier -- `.gitignore` is attacker-controlled and must never be
 * able to hide a payload.
 *
 * 2026-08-07 tiering-trade-off reversal: `classify()` no longer assigns the
 * `bulk-content` class to anything (see `isMarkerConfigMember` below), so
 * this predicate's `false` branch is presently unreachable through real
 * `classify()` output. Kept, not deleted -- `FILE_CLASSES` (`lib/traverse/
 * index.js`) still lists `bulk-content` as a valid class name (the wave
 * spec's `spec.classes['bulk-content'].fileGlobs` field is the data source
 * `isMarkerConfigMember`'s widened predicate reads from, and renaming or
 * removing it is a wave-spec schema change requiring a `specVersion` bump
 * -- see `docs/wave-spec.md`), so a predicate distinguishing it by name
 * remains coherent even though nothing currently produces it.
 */
...
// credential file), `.npmrc`, AND -- as of the 2026-08-07 tiering-trade-off
// reversal (Vitalik review of plan 17-14; see the removal of
// `KNOWN_TIERING_TRADEOFFS[0]` in tests/helpers/chaindrop-corpus.js and
// docs/wave-spec.md) -- every name/glob `spec.classes['bulk-content']
// .fileGlobs` lists. The OLD bash scanner's section 6b marker-string scan
// never consulted gitignore for ANY of its allow-listed extensions, not
// just credential files; D-13's original `bulk-content` tiering silently
// regressed detection of a marker string in a git-ignored path outside
// `PRUNE_COMMON` for every OTHER extension in that list. Every
// marker-string candidate is therefore targeted-tier now, matching bash
// exactly -- which makes the `bulk-content` membership branch below
// UNREACHABLE from this function's own widened predicate (kept, not
// deleted, rather than ripping out the FILE_CLASSES/results.js/engine.js
// plumbing other code still references; see classify.test.js's
// "bulk-content class (now unreachable)" block and
// tests/traverse/gitignore-tiering.test.js for the proof both directions).
function isMarkerConfigMember(name, spec) {
  if (name === '.env' || name.startsWith('.env.') || name === '.npmrc') return true;
  const globs = spec.classes['bulk-content'].fileGlobs;
  return globs.some((g) => matchSimpleGlob(name, g));
}

/**
 * `classify(event, spec, ctx)` -- `event` is a `WalkEvent` (see
 * `lib/traverse/index.js`), `spec` is the parsed wave spec (plan 17-04),
 * and `ctx` carries `{ selfRoot, skips }`: this project's own repo root (so
 ...
 * skip-inventory writes for size outcomes).
 *
 * 2026-08-07 tiering-trade-off reversal: `ctx.ignore` was removed from this
 * contract entirely (previously `{ ignore, selfRoot, skips }`) -- this
 * module no longer consults a gitignore resolver for ANY class. See
 * `isMarkerConfigMember`'s doc comment above and `docs/wave-spec.md`'s
 * tiering-rules section for the full history: `bulk-content` (the one
 * class that ever consulted it) can no longer be assigned by `classify()`,
 * and `lib/traverse/git-ignore.js` -- the module that supplied the
 * resolver -- has been deleted along with its `lib/traverse/engine.js`
 * call sites, since nothing consumes its decisions any more.
 *
 * Returns `{ classes: string[], skipReason: string|null }`. Every class is
 * now a simple present/absent membership test; `skipReason` is retained on
 * the return shape for `lib/traverse/engine.js`'s existing `if
 * (c.skipReason) skips.add(...)` call sites, but `classify()` itself never
 * sets it to anything but `null` -- there is no longer a classify()-level
 * skip decision (media-extension skipping was `bulk-content`-only and was
 * removed with it; oversized/unreadable/binary-sniff skips are read-pool
 * decisions, made after a file is opened, never here).
 */
function classify(event, spec, ctx) {
  ...
  if (ctx.selfRoot && (absPath === ctx.selfRoot || absPath.startsWith(ctx.selfRoot + path.sep))) {
    return { classes: [], skipReason: null };
  }
  ...
  // marker-config: A1 row 6b's full marker-string scan, ALL in the TARGETED
  // tier -- `.env`/`.env.*`/`.npmrc` plus every other name/extension
  // `spec.classes['bulk-content'].fileGlobs` lists (`.js`/`.mjs`/`.ts`/
  // `.json`/etc). No gitignore resolver is ever consulted for any of these
  // names. This restores the OLD bash scanner's section 6b exactly -- it
  // had no gitignore awareness at all for ANY of its allow-listed
  // extensions, not just credential files. See `isMarkerConfigMember`'s
  // doc comment above for the full 2026-08-07 history (a narrower version
  // of this class, `.env`/`.env.*`/`.npmrc` only, previously coexisted
  // with a separate gitignore-consulting `bulk-content` class for the
  // other extensions; that split was reviewed and reverted as a real
  // detection regression, not an acceptable trade-off).
  if (isMarkerConfigMember(name, spec) && !crossesPrunedDir(absPath, MARKER_CONFIG_PRUNE)) {
    classes.push('marker-config');
  }

  return { classes, skipReason };
}
