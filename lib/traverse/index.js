...
// ---------------------------------------------------------------------
// D-01 (18-CONTEXT.md, G-1543/G-1544, added 2026-08-12): `swapped` is the
// SIXTH reason. It is ANOMALY (lib/traverse/engine.js's
// ANOMALY_SKIP_REASONS) while `symlink` stays SCOPE -- the two used to
// share the `symlink` bucket, which is exactly why this could not be
// fixed by reclassifying: a shared counter cannot tell a routine
// enumeration-time symlink refusal (368 of 368 measured on a real
// 424,561-file tree) apart from a post-classification TOCTOU swap (0 of
// 368 measured on the same tree) -- promoting the shared bucket would
// exit 2 on a clean developer machine. Placed LAST so the existing five
// keep their positions; nothing indexes this array, but a stable order
// keeps `findings.json` diffs readable.
