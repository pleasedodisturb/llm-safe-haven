...
function createReadPool(options) {
  ...
  async function processOne(work) {
    ...
    try {
      ...
      // O_NOFOLLOW refused a symlink at the final path component. This is
      // a TOCTOU swap (D-01, G-1543/G-1544): `walk.js` classified this
      // path with an `lstat` as a REGULAR file at time T1; by the time
      // THIS open() ran at time T2, the path resolved to a symlink. That
      // is different from the walk's OWN symlink refusals -- a symlink
      // the walk itself SEES and refuses during enumeration
      // (walk.js:188,288) -- which keep the `symlink` reason and stay
      // SCOPE, unchanged.
      //
      // Recorded as `swapped` (was `symlink` before this change --
      // D-17.1-E's original single-bucket design, now superseded).
      // `swapped` is ANOMALY (lib/traverse/engine.js's
      // ANOMALY_SKIP_REASONS) because the exit semantics differ: an
      // ordinary enumeration-time symlink is a routine, disclosed scope
      // boundary, but a file that PASSED classification and then became
      // a symlink before it was opened means the tree changed underneath
      // the scan -- exactly what D-03 requires before a new skip reason
      // is justified.
      //
      // Platform note: on a hypothetical platform returning `EMLINK`
      // instead of `ELOOP` (FreeBSD does this deliberately; darwin does
      // NOT -- measured on this machine), this branch falls through to
      // the generic `unreadable` case below, which after this change is
      // ALSO ANOMALY -- fail-closed with a slightly wrong label, not a
      // silent miss. No `EMLINK` branch is added deliberately: no CI leg
      // (ubuntu/Node 22, ubuntu/Node 18, macos-latest/Node 22) can
      // exercise it, and an untestable branch would violate this repo's
      // own reasoning-not-evidence standard.
      //
      // Every other open()-time failure (ENOENT/EACCES/etc.) is unchanged.
      if (err.code === 'ELOOP') {
        recordSkip('swapped', absPath);
      } else {
