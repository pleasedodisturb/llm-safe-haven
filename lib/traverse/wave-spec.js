...
//
// FIFTH iteration: the construct must be NESTED inside a bracket expression.
// The previous form matched a bare `[:x:]` anywhere, which had two costs:
// it missed the empty-named constructs `[[..]]`, `[[::]]`, `[[==]]`, and it
// false-positived on `[.-.]` — a valid, if degenerate, JS range from dot to
// dot. Requiring an enclosing `[` distinguishes a POSIX construct (which only
// ever appears INSIDE a bracket expression) from a standalone JS class that
// merely uses the same punctuation. Result: 0 false positives across a
// 24-pattern legit corpus, versus 1 before.
//
// A reviewer proposed `/\[([:.=]).*?\1\]/` instead. Measured, that catches
// one more exotic shape but reintroduces `[::]`, `[..]` and `[==]` as false
// positives — the exact regression the fourth iteration had just fixed. It
// was rejected for that reason. A ban that rejects valid detectors is a
// scanner outage, which is worse than missing a shape nobody writes.
//
// KNOWN RESIDUAL, accepted rather than chased:
//   [][:alpha:]]        a class whose first member is `]`. In JS `[]` matches
//                       nothing at all, so the pattern is already inert for
//                       reasons unrelated to POSIX.
//   [\u003Aalpha\u003A]  the delimiter written as a unicode escape. Anyone
//                       who knows to write \u003A knows JS has no POSIX
//                       classes; catching it would require normalising escape
//                       sequences before matching.
// Both are documented blind spots, not oversights. See the test file.
//
// FOURTH iteration: the inner quantifier is `+`, not `*`. With `*` the
// expression matched `[::]`, `[..]` and `[==]` — which are ordinary JS
// character classes matching a literal colon, dot or equals, and are exactly
// the kind of thing a real detector writes. Requiring non-empty inner content
// means a POSIX construct must actually name something. Found by CodeRabbit
// on PR #96; my own legit-pattern corpus had tested `[:]`, `[.]` and `[=]`
// but never the doubled forms.
//
