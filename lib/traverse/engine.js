 ...
 * state, the ignore-resolver cache, the skip inventory and the shared
 * budget across two phases of asynchronous work -- state a plain function
 * would otherwise have to thread through every call by hand. Everything
 * else exported here (`traverse`, `DETECTOR_OWNERSHIP`) is a plain function
 * or frozen data, matching the rest of this codebase.
 *
 * This module composes `walk.js` (one physical pass per root), `classify.js`
 * (per-class membership + tiering), `git-ignore.js` (the bulk-tier-only
 * gitignore consult), `read-pool.js` (the bounded, single-open-per-path
 ...
 *
 * A git degradation (see `git-ignore.js`) fails OPEN for the bulk tier and
 * is reported in `degradations` -- it never sets `incomplete`.
 *
...
const { createIgnoreResolver } = require('./git-ignore.js');
...
const DETECTOR_OWNERSHIP = Object.freeze(
  [
    ...
    },
    {
      ...
      rationale: 'Traversal-driven, bulk tier plus the targeted marker-config class (the credential-file half of this same check, kept out of the gitignore-prunable tier -- see D-13 in lib/traverse/index.js).',
    },
    {
      section: '6c',
 ...
 * module also writes classify-driven (`gitignored` / `media`) and
 * read-pool-driven (`oversized` / further `unreadable` / `budget`) skips
 * into, so the caller sees ONE combined ledger.
 */
...
// A no-op ignore resolver used by `enumerateSync()` whenever `bulk-content`
// is not among the requested classes -- `classify()` unconditionally
// evaluates every class's membership predicate in a single call, including
// the bulk-content branch, so avoiding a real `createIgnoreResolver()` (and
// therefore avoiding ANY subprocess) requires supplying a resolver whose
// answer is never actually consulted downstream, not merely deferring the
// real one. Its `isBulkEligible` answer is unconditionally eligible; that
// is safe here specifically because the caller never reads the
// `bulk-content` bucket of the result when it was not requested.
const NOOP_IGNORE = Object.freeze({
  isBulkEligible: () => ({ eligible: true, reason: null }),
  indexFor: () => ({ degraded: null, listed: new Set() }),
  degradations: () => ({}),
  stats: () => ({ repos: 0, subprocesses: 0, cacheHits: 0 }),
});
...
// bulk-content) simply never match anything -- classes that need no spec
// data at all (env-secrets, marker-config, agent-config) are unaffected.
...
function generateContentFindings({ spec, workMap, resultsByPath, hashCandidateNames, findings }) {
  ...
  for (const [absPath, rec] of workMap) {
    ...
    // --- section 6b: marker strings (marker-config OR bulk-content) ------
    const markerClass = rec.classes.includes('marker-config') ? 'marker-config' : rec.classes.includes('bulk-content') ? 'bulk-content' : null;
    if (markerClass && text && markerRe.test(text)) {
      findings.push(mkFinding('marker-string', markerClass, absPath, 'ChainDrop marker string found', 'fail'));
    }
  }
}
...
class Traversal {
   ...
   * is spawned unless `bulk-content` is among the requested classes --
   * `spec` may be omitted entirely when every requested class needs no
   * spec DATA (e.g. `env-secrets`, the class `lib/scan.js` consumes).
   */
  enumerateSync() {
    const spec = this.spec || EMPTY_SPEC;
    const requestedSet = new Set(this.classes);
    const needsIgnore = requestedSet.has('bulk-content');
    // `options.ignore` is an injectable-seam override (matching the existing
    // fs/spawnSync/now/budget pattern), used by tests to wrap the resolver
    // and record which repoRoot each `indexFor` call was made with.
    const ignore = this.rawOptions.ignore || (needsIgnore ? createIgnoreResolver(this.rawOptions) : NOOP_IGNORE);
    ...
    const walkResult = walk(this.roots, this.rawOptions, (event) => {
      ...
      const c = classify(event, spec, { selfRoot: this.selfRoot, ignore, skips });
    ...
    return { byClass, skips, counts: walkResult.counts };
  }
  ...
  async run() {
    ...
    const ignore = this.rawOptions.ignore || createIgnoreResolver({ ...this.rawOptions, budget });
    ...
    let targetedComplete = !walkResult.stopped;
    let bulkComplete = !walkResult.stopped;
    ...
    if (targetedComplete) {
      ...
      // D-20 phase-boundary recheck. `budget.js`'s clock is advanced only by
      // `noteDirectory()`/`noteFile()`, both called exclusively from
      // `walk.js` -- `read-pool.js` only ever OBSERVES the already-latched
      // flag via `exhausted()`, it never advances the clock itself (see this
      // file's module header). Without an explicit recheck here, real
      // wall-clock time spent on phase 1's I/O would never be noticed before
      // committing to the bulk tier. `noteDirectory()` is reused rather than
      // a bespoke API because it is budget.js's only public "recheck the
      // live clock and possibly latch" entry point; its `dirsWalked`
      // side-effect is not exposed on `TraverseResult` and is harmless here.
      budget.noteDirectory();
      if (budget.exhausted()) {
        // The targeted tier itself already finished draining above -- only
        // the bulk tier, which has not started yet, is affected.
        bulkComplete = false;
      } else {
        ...
        budget.noteDirectory(); // symmetric post-phase recheck
        if (budget.exhausted()) bulkComplete = false;
      }
    }
    ...
    const incomplete = !targetedComplete || !bulkComplete;
    const exitCode = computeExit({ severityCounts, incomplete });
    const degradations = Object.values(ignore.degradations());
