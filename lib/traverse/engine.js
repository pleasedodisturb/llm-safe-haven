...
/**
 * D-02a (17.1-CONTEXT.md): the blanket `oversized > 0 -> incomplete` rule
 * (D-02/G-1512, plan 17.1-01 Task 3) was measured against a real
 * 345,487-file tree and REJECTED ON EVIDENCE -- 134 oversized skips, ZERO
 * in a targeted-content class, 0 unreadable, 0 budget, both tiers
 * complete, yet `incomplete: true` / exit 2 solely because of generic
 * `bulk-content` marker-string reads skipped for size -- a scope boundary
 * this engine inherits from the original bash scanner (`-size -256k`),
 * not evidence the scan failed to finish. Crying wolf on a clean tree
 * costs the exit-2 signal for the `unreadable`/budget cases that
 * genuinely matter.
 *
 * Revised rule: an `oversized` skip counts toward `incomplete` only when
 * the skipped file was a NAMED CANDIDATE:
 *   - a targeted-content read (`bulkSkipped === 'oversized-bulk'`) on a
 *     record whose classes intersect `TARGETED_CONTENT_CLASSES`
 *     (no-prune, family-packages, agent-config, marker-config); OR
 *   - a hash read (`hashSkipped === 'oversized-hash'`) -- `needHash` is
 *     ONLY ever true for a hash-candidate name (see `buildWorkFlags`
 *     above), so every `oversized-hash` skip is, by construction, already
 *     a candidate; no further class check is needed for that branch.
 *
 * A poisoned `package-lock.json`, `.env`, or any other targeted-content/
 * hash-candidate file padded past its cap still trips this -- G-1512's
 * actual reproduction, and both directions are guarded (see
 * `tests/traverse/engine.test.js`'s Guard 3 pair). A generic
 * `bulk-content` file skipped purely for size does not: `bulk-content`
 * itself is deliberately excluded even though `needBulk` can reach the
 * SAME `oversized-bulk` site as `needTargetedContent` -- `bulk-content`
 * is presently a structural no-op (see this file's module header) and,
 * even if a future wave-spec class revives it, it was never one of the
 * four `TARGETED_CONTENT_CLASSES` this rule scopes to.
 *
 * Reads `bulkSkipped`/`hashSkipped` off each read-pool result -- fields
 * `processOne()` in `read-pool.js` already returns per record; nothing in
 * `read-pool.js` changes. `SKIP_REASONS` (still a flat `oversized`
 * bucket) and the results-dir schema (`skips/oversized.z`, `scalars/
 * skip-oversized`) are both UNCHANGED -- this function computes a
 * separate, engine-local count consumed only by the `incomplete`
 * expression below, never written to the results directory or the skip
 * inventory itself.
 */
function countCandidateOversizedSkips(workMap, resultsByPath) {
  let count = 0;
  for (const [absPath, rec] of workMap) {
    const result = resultsByPath.get(absPath);
    if (!result) continue;
    if (result.bulkSkipped === 'oversized-bulk' && rec.classes.some((c) => TARGETED_CONTENT_CLASSES.has(c))) {
      count += 1;
    }
    if (result.hashSkipped === 'oversized-hash') {
      count += 1;
    }
  }
  return count;
...
class Traversal {
  ...
  async run() {
    ...
    // G-1501 / G-1512 (TRAV-15, decisions D-01/D-02/D-02a). Six explicit points:
    ...
    // (c) `oversized` joins it per decision D-02 (G-1512): a file too large
    //     to examine is not examined. D-02a (17.1-CONTEXT.md, operator
    //     decision after a real-tree measurement) SCOPED this from a
    //     blanket `skips.counts().oversized > 0` to
    //     `countCandidateOversizedSkips(...) > 0` -- see that function's
    //     doc comment above for the full rationale and the measurement
    //     that drove it. Only a NAMED CANDIDATE's oversized skip
    //     (targeted-content class member or hash candidate) counts; a
    //     generic bulk-content size skip does not.
    // (d) `budget` already propagates via the tiers above; `symlink` and
    //     `other-device` are DELIBERATELY excluded here (D-06
    ...
    // (e) the D-24 hash cap and the bulk read cap themselves are UNCHANGED
    //     by this -- this is about reporting, not about raising a cap.
    // (f) operator-facing consequence: a scan root containing a
    //     pre-existing permission-restricted path, or a targeted-content/
    //     hash-candidate file too large to read, now exits 2. The
    //     offending paths are still enumerated in full (candidate or not)
    //     in the retained results dir at skips/unreadable.z and
    //     skips/oversized.z (lib/traverse/results.js:267), with counts at
    //     scalars/skip-unreadable and scalars/skip-oversized -- those
    //     artifacts are unchanged; only which `oversized` skips flip the
    //     exit code is scoped.
    const candidateOversizedCount = countCandidateOversizedSkips(workMap, resultsByPath);
    const incomplete = !targetedComplete || !bulkComplete || skips.counts().unreadable > 0 || candidateOversizedCount > 0;
...
// `countCandidateOversizedSkips` and `TARGETED_CONTENT_CLASSES` are exported
// SOLELY for direct unit testing of the D-02a scoping rule (see that
// function's doc comment above). This is deliberate, additive test-only
// surface, not a new public API: `classify()` cannot currently produce a
// `bulk-content`-classified `WalkEvent` (2026-08-07 tiering-trade-off
// reversal, `lib/traverse/classify.js`'s module header), so no REAL
// `Traversal.run()` call over a real filesystem can exercise the
// "non-candidate oversized skip" branch end-to-end today -- proven on the
// real tree in `17.1-01-SUMMARY.md`'s D-02a correction section. A direct
// unit test of this function is therefore the only way to prove the
// scoping logic itself is correct and load-bearing, independent of
// whether any reachable real-world input currently exercises its negative
// branch.
module.exports = { Traversal, traverse, DETECTOR_OWNERSHIP, countCandidateOversizedSkips, TARGETED_CONTENT_CLASSES };
