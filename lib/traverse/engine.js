 ...
 * `budget` / `swapped`) skips into, so the caller sees ONE combined ledger. `classify()`
...
// ---------------------------------------------------------------------
// ANOMALY_SKIP_REASONS / SCOPE_SKIP_REASONS (D-02b, 17.1-CONTEXT.md,
// G-1512 final form -- supersedes the blanket D-02 form AND the D-02a
// candidate-scoping correction, both implemented and measured in plan
// 17.1-01 and both retired here; see `17.1-01-SUMMARY.md`'s "D-02a
// Correction" and "D-02b" sections for the full history).
//
// `incomplete` was being asked to carry two different meanings under one
// ad-hoc per-reason expression, and that conflation produced a real
// crying-wolf failure mode: D-02's blanket form measured `incomplete: true`
// / exit 2 on a completely clean 345,487-file real tree (0 unreadable, 0
// budget, both tiers complete) solely because of 134 routine oversized
// files. D-02a's candidate-scoping fix was then measured and found to be a
// STRUCTURAL NO-OP against the same real tree (the 2026-08-07
// tiering-trade-off reversal widened `marker-config` to cover every
// allow-listed extension, so "named candidate" now means "almost any text
// file" -- see the module-exports comment near the bottom of this file,
// where the retired `countCandidateOversizedSkips` helper's removal is
// explained).
//
// G-1512's actual premise -- that a poisoned lockfile could hide from
// detection past the 256 KiB read cap -- was DISPROVED BY TEST: poisoned-
// version matching is bash-owned and unbounded (`poisoned_hit_in_file()`
// awk-matches every path in `lists/lockfiles.z` with no size guard), so a
// 568 KiB poisoned `package-lock.json` -- 2.2x the cap -- is still
// `[FAIL]`-detected, exit 1. The only detection actually lost to the cap is
// a marker STRING inside a >256 KiB file -- the identical blind spot the
// pre-retrofit bash scanner had while reporting exit 0. That is a
// deliberate, inherited scope boundary, not evidence a scan failed.
//
// D-02b's permanent fix: split the two meanings explicitly.
//
//   ANOMALY -- "I tried to look and could not." Rare, actionable, the
//   result is untrustworthy. Feeds `incomplete`.
//   SCOPE -- "I deliberately did not look there." Routine, by design,
//   disclosed in output (`[skip] oversized: N`, `skips/oversized.z`,
//   `scalars/skip-oversized`) but never anomalous. Never feeds
//   `incomplete`.
//
// Every SKIP_REASONS member MUST belong to EXACTLY ONE bucket --
// `tests/traverse/engine.test.js`'s partition test asserts this directly
// against these two sets, so a sixth skip reason forces an explicit
// classification here before it can silently do (or not do) anything to
// the exit code.
//
// D-01 (18-CONTEXT.md, G-1543/G-1544, 2026-08-12): the SIXTH SKIP_REASONS
// member, `swapped`, is exactly the case that classification requirement
// anticipated. A file that PASSED `walk.js`'s classification as a
// regular file and was a SYMLINK by the time
// `lib/traverse/read-pool.js`'s `open()` ran is a TOCTOU swap between the
// classifying `lstat` and this open -- not a scope decision, so it joins
// ANOMALY. Measured on a real 424,561-file tree: 368 of 368 `symlink`
// skips originated at the walk's own enumeration sites (`walk.js:188,288`),
// zero at the post-classification `open()` site -- the near-exact match
// against `find <tree> -type l` (369) proves the walk accounts for
// essentially all of them. The two cases used to share one `symlink`
// bucket, which is exactly why this could not be settled by
// reclassifying: a shared counter cannot tell a routine enumeration-time
// refusal apart from a real TOCTOU swap. Promoting the whole `symlink`
// bucket to ANOMALY (the literal reading of G-1543/G-1544) would exit 2
// on a clean developer machine -- a worse crying-wolf regression than the
// 134 oversized files that forced the D-02b reversal above. `other-device`
// deliberately does NOT join ANOMALY here (D-02): measured 0 on a machine
// with no cross-device mounts, so its real-world frequency is unknown and
// the crying-wolf risk is unquantified; it is also a scope decision made
// at enumeration, the same distinction that justifies `swapped`. A future
// reader must not "complete the set" by folding it in too.
// ---------------------------------------------------------------------
const ANOMALY_SKIP_REASONS = Object.freeze(new Set(['unreadable', 'budget', 'swapped']));
const SCOPE_SKIP_REASONS = Object.freeze(new Set(['oversized', 'symlink', 'other-device']));

// `budget`'s ANOMALY channel is tier completeness (`targetedComplete` /
// `bulkComplete`, computed in `run()` below), not a raw
// `skips.counts().budget` read. A `budget` skip recorded during the walk or
// either read-pool phase is exactly what makes `budget.exhausted()` true,
// which is exactly what blocks `tierComplete()` from ever being called for
// that tier (D-06/B1's phase-boundary-ordering fix, plan 17.1-01 Task 2) --
// so tier completeness ALREADY carries the `budget` anomaly's signal;
// separately reading `skips.counts().budget` would double-count the same
// underlying event, not detect a new one. Every OTHER ANOMALY reason
// (today `unreadable` and `swapped`) has no such pre-existing channel and
// is counted directly off the skip inventory by `countAnomalySkips` below.
// The set is DERIVED from ANOMALY_SKIP_REASONS, so it stays correct without
// an edit here; only this prose needed updating when `swapped` landed.
const ANOMALY_REASONS_VIA_SKIP_INVENTORY = new Set([...ANOMALY_SKIP_REASONS].filter((r) => r !== 'budget'));

/**
 * Sums the skip-inventory counts for every ANOMALY reason whose signal is
 * NOT already carried by tier completeness (see the comment above) --
 * today, that is `unreadable` and `swapped`. Driven by `ANOMALY_SKIP_REASONS`
 * rather than a hardcoded `unreadable` literal, so a future ANOMALY reason
 * added to that set is automatically wired into `incomplete` (via `run()`
 * below) without a second edit at the call site.
 */
function countAnomalySkips(skipCounts) {
  let count = 0;
  for (const reason of ANOMALY_REASONS_VIA_SKIP_INVENTORY) {
...
function generateContentFindings({ spec, workMap, resultsByPath, hashCandidateNames, findings }) {
  ...
  // NOTE: commandPattern/failPattern (POSIX ERE, `[[:space:]]`) are for
  // scripts/scan-chaindrop-aug2026.sh's `grep -E` consumer only. A JS
  // RegExp does not understand POSIX bracket classes -- `[[:space:]]`
  // there is a literal character class of `[`,`:`,`s`,`p`,`a`,`c`,`e`,`]`,
  // not whitespace -- so the engine MUST consume the js*Pattern siblings
  // (G-1482 merge-blocking fix, 2026-08-10). Do not revert to
  // commandPattern/failPattern here.
  const installMarkerRe = new RegExp(spec.installMarker.jsPattern);
  const commandSusRe = new RegExp(spec.persistence.claudeSettings.jsCommandPattern);
  const vscodeTriggerRe = new RegExp(spec.persistence.vscodeTasks.triggerPattern);
  const vscodeFailRe = new RegExp(spec.persistence.vscodeTasks.jsFailPattern, 'i');
...
class Traversal {
  ...
  async run() {
    ...
    // G-1501 / G-1512 (TRAV-15, decision D-02b -- FINAL, 17.1-CONTEXT.md).
    // `incomplete` is now driven structurally by the ANOMALY_SKIP_REASONS /
    // SCOPE_SKIP_REASONS classification defined above, not an ad-hoc
    // per-reason expression:
    // (a) this restores the project's standing locked rule from
    //     .planning/STATE.md -- "Incomplete scan -> exit 2, never 0" -- for
    //     every ANOMALY reason.
    // (b) `unreadable` (D-01, via `countAnomalySkips`) -- every unreadable
    //     path, directory OR file -- is what ROADMAP criterion 1's wording
    //     requires; decision D-01 rejected narrowing it to directories-only
    //     or adding a `--lenient` opt-out in this phase. UNCHANGED by D-02b.
    // (c) `budget` (via tier completeness -- see the comment on
    //     `ANOMALY_REASONS_VIA_SKIP_INVENTORY` above) -- the budget
    //     anomaly's own channel.
    // (d) `oversized` -- FINAL, D-02b: does NOT affect the exit code, full
    //     stop. Both the blanket D-02 form and the D-02a candidate-scoping
    //     correction (plan 17.1-01 Task 3 and its follow-on) were measured
    //     on a real tree and superseded -- see `ANOMALY_SKIP_REASONS`'s doc
    //     comment above and `17.1-CONTEXT.md`'s D-02b entry for the full
    //     account, including why G-1512's underlying premise was disproved
    //     by test. `oversized` is a SCOPE reason: routine, by design,
    //     disclosed (`skips/oversized.z`, `scalars/skip-oversized`,
    //     `[skip] oversized: N` in scanner output) but never anomalous.
    // (e) `symlink` / `other-device` -- also SCOPE, unchanged: D-06
    ...
    // (e2) `swapped` -- ANOMALY, added 2026-08-12 (D-01, G-1543/G-1544): a
    //     file that PASSED classification as a regular file and was a
    //     SYMLINK by the time the read pool opened it
    //     (lib/traverse/read-pool.js's ELOOP branch) is a TOCTOU swap, not
    //     a scope decision -- unlike (e)'s `symlink`, which is refused
    //     during enumeration and never reaches the read pool at all.
    //     Measured on a real 424,561-file tree: 368 of 368 symlink skips
    //     originated at the walk, zero at the post-classification open()
    //     site, so the two cases had to be counted separately before
    //     either could affect the exit code -- the shared `symlink`
    //     bucket could not tell them apart. `other-device` deliberately
    //     does NOT join this set (D-02): measured 0 on a machine with no
    //     cross-device mounts, so its real-world frequency is unknown and
    //     the crying-wolf risk is unquantified; a device boundary is also
    //     a scope decision made at enumeration, like `symlink`. Do not
    //     "complete the set" here either.
    // (f) the D-24 hash cap and the bulk read cap themselves are UNCHANGED
    //     by this -- this was always about reporting, never about raising a
    //     cap.
    // (g) operator-facing consequence of D-02b (relative to D-02/D-02a): an
    //     oversized file no longer flips the exit code by itself. A scan
    //     root containing a pre-existing permission-restricted path (D-01)
    //     still exits 2; a scan root whose only anomaly is oversized
    //     content now exits 0 or 1 purely on its findings, matching the
    //     pre-17.1 baseline for that one dimension. The offending paths are
    //     still enumerated in full in the retained results dir at
    //     skips/oversized.z (lib/traverse/results.js:267), with counts at
    //     scalars/skip-oversized -- those artifacts and the results-dir
    //     schema are unchanged; only whether `oversized` can flip the exit
    //     code changed.
    const incomplete = !targetedComplete || !bulkComplete || countAnomalySkips(skips.counts()) > 0;
...
// D-02b (17.1-CONTEXT.md) retired `countCandidateOversizedSkips` --
// `oversized` no longer feeds `incomplete` under any scoping rule, so the
// D-02a candidate-scoping helper it existed to drive has no remaining call
// site anywhere in `run()`. It is REMOVED, not left dead: keeping an
// unused function wired to nothing invites a future reader to assume it is
// still load-bearing, and its own removal is the load-bearing proof that
// nothing else in this file or `tests/` depended on it (confirmed by grep
// before removal -- see the D-02b summary section this change appends to
// `17.1-01-SUMMARY.md`).
//
// `TARGETED_CONTENT_CLASSES` is KEPT and re-exported -- unlike
// `countCandidateOversizedSkips`, it is still LIVE production logic
// (`buildWorkFlags`'s `needTargetedContent` above), not a D-02a-only
// artifact, and exporting it lets a test assert its membership directly
// without duplicating the four-class literal.
//
// `ANOMALY_SKIP_REASONS` / `SCOPE_SKIP_REASONS` are exported for the
// SKIP_REASONS partition test (`tests/traverse/engine.test.js`, D-02b) --
// see the doc comment where they are defined above for the classification
// these two sets encode and why every `SKIP_REASONS` member must belong to
// exactly one of them.
module.exports = { Traversal, traverse, DETECTOR_OWNERSHIP, TARGETED_CONTENT_CLASSES, ANOMALY_SKIP_REASONS, SCOPE_SKIP_REASONS };
