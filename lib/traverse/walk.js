...
// How often (in Dirents ITERATED, walk-wide, across every directory) the
// entry loop in `walkDirectory` re-checks the wall clock via the existing
// `noteDirectory()` API (G-1513, TRAV-16).
//
// Why this exists at all: `noteDirectory()` -- the only wall-clock read on
// the ordinary fast path (Dirent type known via `d_type`) -- fires once per
// DIRECTORY, not once per entry, deliberately (see budget.js's header). A
// single flat directory of hundreds of thousands of entries (a flat
// `node_modules` install, a maildir, a build-output directory) reads the
// clock exactly ONCE while emitting all of them, so `LSH_BUDGET_SECONDS`
// could be exceeded by an unbounded margin inside that one directory. This
// is the same defect class G-1506 fixed one tier earlier in
// `read-pool.js`'s drain(), and the same shape `LSTAT_FALLBACK_CLOCK_INTERVAL`
// above already guards -- but only on the DT_UNKNOWN branch, leaving the
// common fast path exposed.
//
// Why 512, justified on a worst-case-overrun bound (NOT on any test staying
// green -- see the B1 postmortem in 17.1-CONTEXT.md for why that reasoning
// is forbidden here):
//   - The clock is re-read every WALK_ENTRY_CLOCK_INTERVAL entries iterated,
//     walk-wide (not per-directory -- see the `entryScanCount` comment on
//     the `ctx` literal below for why).
//   - Worst case once the wall-clock budget has truly expired: at most 512
//     further entry iterations before the next recheck. The dominant
//     per-entry cost on this path is at most one `lstatSync` (the D-12
//     device check for a subdirectory entry; a file entry costs no syscall
//     at all once `d_type` is known).
//   - On a pathologically slow mount -- the same hazard
//     `LSTAT_FALLBACK_CLOCK_INTERVAL` exists for -- an lstat can take
//     ~10ms, so the bound is ~= 512 x 10ms ~= 5.1s, ~= 8.5% of the default
//     60s budget. On local APFS/ext4 (lstat ~= 5-20us) the same bound is
//     ~= 3-10ms, i.e. immeasurable.
//   - The cost side: one `process.hrtime.bigint()` plus one BigInt divide
//     per 512 entries. On the recorded 418,728-entry real tree (17.1-06)
//     that is ~= 818 extra clock reads, against 418,728 if the check were
//     moved into the per-entry checkpoint instead -- which T-17-04-04
//     forbids for exactly that reason (a per-entry clock read costs real
//     time across hundreds of thousands of entries for no correctness gain
//     on this path).
//   - Secondarily (NOT the justification): this matches
//     `LSTAT_FALLBACK_CLOCK_INTERVAL = 512`, one cadence constant to reason
//     about for the same hazard.
//   - This value was NOT chosen to keep any existing assertion green. The
//     structural cadence assertion in `tests/traverse/traverse-structural.test.js`
//     is a formula derived from this constant and the fixture, and a
//     fixture larger than the interval is what actually exercises the
//     recheck.
const WALK_ENTRY_CLOCK_INTERVAL = 512;
...
function walkDirectory(dirPath, depth, ctx) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Interval-cadence wall-clock recheck (G-1513, TRAV-16) -- see the
      // WALK_ENTRY_CLOCK_INTERVAL comment above for the worst-case-overrun
      // justification. `ctx.entryScanCount` is walk-wide and incremented at
      // the top of the loop body, BEFORE any `continue`, so it equals the
      // number of Dirents ITERATED (a quantity a test can compute directly
      // from a fixture), not the number emitted or recursed into.
      // `noteDirectory()` is reused deliberately -- it is the budget's only
      // public "recheck the live clock and possibly latch" entry point, and
      // its own internal `dirsWalked` increment (confirmed by reading
      // budget.js) is a different counter than `ctx.dirsWalked` above, which
      // is the one `walkResult.counts.dirsWalked` reports -- so these extra
      // calls cannot inflate that reported count.
      ctx.entryScanCount += 1;
      if (ctx.entryScanCount % WALK_ENTRY_CLOCK_INTERVAL === 0) {
        if (!ctx.budget.noteDirectory()) {
          ctx.skips.add('budget', dirPath);
          break;
        }
 ...
 *
 * G-1504 -- this call uses `statSync` (follows symlinks), the ONLY stat call
 * in this entire file that does. Every other stat in this module (the
 * DT_UNKNOWN fallback, the per-directory device check) never follows a link
 * and MUST stay that way -- D-06 (never follow symlinks during the walk) is
 * fully intact; this root-level follow does not relax it. The root is an
 * explicitly configured entry point, the same class of exception D-12
 * already carves out for `LSH_ROOTS`-opted-in external volumes: anchoring
 * `ctx.rootDev` on the LINK (a no-follow stat) rather than the TARGET meant
 * a symlinked root's device never matched any of its own real
 * subdirectories' devices, so `walkDirectory`'s device-containment check at
 * line ~250 pruned the entire target subtree as `other-device` on every
 * symlinked root -- the operator believed a volume was scanned when only
 * its top-level symlink entry was examined. Symlinks encountered DURING
 * recursion (any entry below the root) are untouched by this change and
 * still go through `walkDirectory`'s `entry.isSymbolicLink()` skip and
 * `resolveEntryType`'s no-follow DT_UNKNOWN fallback, both unchanged. This
 * follow happens EXACTLY ONCE per root, before the walk begins -- it cannot
 * introduce a traversal loop: nothing recurses back into `walkOneRoot`, and
 * the walk is additionally `maxDepth`-bounded via `walkDirectory`'s own
 * guard.
 */
function walkOneRoot(root, ctx) {
  let rootStat;
  try {
    rootStat = ctx.options.fs.statSync(root);
  } catch {
...
function walk(roots, options, visit) {
  ...
  const ctx = {
    ...
    // Walk-WIDE (not per-directory) Dirent-iteration counter feeding the
    // WALK_ENTRY_CLOCK_INTERVAL recheck in `walkDirectory`'s entry loop
    // (G-1513). Deliberately not reset per directory: a tree of very many
    // SMALL directories is already covered by the per-directory
    // `noteDirectory()` call, while a walk-wide counter additionally bounds
    // any MIXTURE of shapes (e.g. thousands of small directories followed
    // by one enormous flat one). A per-directory counter would reset to 0
    // at the top of exactly the directory a mixed tree needs it not to.
    entryScanCount: 0,
    rootDev: null,
    repoStack: [],
  };
...
module.exports = { walk, walkRoot, WALK_ENTRY_CLOCK_INTERVAL };
