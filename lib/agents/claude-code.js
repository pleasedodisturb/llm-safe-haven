...
const HOOK_FILES = ['bash-firewall.js', 'secret-guard.js', 'config-guard.js', 'audit-logger.js'];
...
const HOOK_CONFIG = {
  PreToolUse: [
    ...
    {
      matcher: 'Write|Edit|MultiEdit',
      hooks: [{ type: 'command', command: `node ${path.join(HOOKS_DIR, 'config-guard.js')}`, timeout: 5 }],
    },
  ],
  PostToolUse: [
    {
...
function mergeSettings(flags) {
  ...
  // BUG-2: Respect dryRun flag — never write settings in dry-run mode
  if (flags.dryRun) {
    actions.push('[dry-run] Would merge hook config into ' + SETTINGS_PATH);
    return { actions, warnings };
  }
  ...
  // M-1: Prototype pollution guard — reject non-plain objects from untrusted JSON
  if (typeof existing !== 'object' || Array.isArray(existing) || existing === null) {
    existing = {};
  }
  delete existing.__proto__;
  delete existing.constructor;
  ...
  if (changed) {
    ...
    // H-2: Atomic write — write to temp file then rename (atomic on POSIX)
...
module.exports = {
  ...
  id: 'claude-code',
  tier: 1,
  // Declared capabilities (G-1661, plan 21-06/21-07) -- consumed by
  // computeExpectedTier() in base.js. The only module installing an
  // enforced hook system: harden() copies HOOK_FILES into ~/.claude/hooks
  // and merges HOOK_CONFIG into ~/.claude/settings.json (above). No ignore
  // file is written, so ignoreFileHonored/ignoreFileCitation are N/A (null).
  installsEnforcedHooks: true,
  writesIgnoreFile: false,
  ignoreFileHonored: null,
  ignoreFileCitation: null,
  detect,
  harden,
  audit,
  // Exposed for testing
