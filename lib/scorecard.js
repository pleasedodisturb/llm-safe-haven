...
const MCP_SEVERITY_ORDER = { critical: 0, high: 1, medium: 2, low: 3, info: 4 };
const MCP_SEVERITY_LABEL = { critical: 'CRITICAL', high: 'HIGH', medium: 'MEDIUM', low: 'LOW', info: 'INFO' };
...
function printMcpFindingLine(finding) {
  const label = MCP_SEVERITY_LABEL[finding.severity] || String(finding.severity).toUpperCase();
  if (finding.confidence === 'unverified') {
    // D-06: dim/neutral only — no C.red/C.yellow anywhere on this line.
    console.log(`      ${MCP_UNVERIFIED_GLYPH} ${C.dim}${label} ${finding.id} — ${finding.message}${C.reset}`);
  } else {
    console.log(`      ${mcpSeverityGlyph(finding.severity)} ${label} ${finding.id} — ${finding.message}`);
  }
}
...
function printMcpFindingGroup(findings) {
  const verified = sortFindingsBySeverity(findings.filter((f) => f.confidence !== 'unverified'));
  const unverified = sortFindingsBySeverity(findings.filter((f) => f.confidence === 'unverified'));
...
function printMcpScan(envelope) {
  ...
  if (findings.length === 0) {
    ...
    console.log(`    ${FAIL} ${C.red}${findings.length} finding(s)${C.reset}`);
    console.log('');
    ...
    for (const key of groupOrder) {
      ...
      const scopeLabel = group.scope ? ` (${group.scope})` : '';
      console.log(`    ${C.bold}${group.agentId} › ${group.serverName || 'unknown'}${scopeLabel}${C.reset}`);
  ...
  if (unfinishedSources.length > 0) {
    ...
    for (const source of unfinishedSources) {
      console.log(`      ${WARN} ${source.agentId} (${source.scope}) — ${source.status}`);
    }
  }
