...
function printAgentSection(agent, detected, hardenResult, auditResult) {
  ...
  // G-1658: surface an EOS/unmaintained notice for a detected agent. Static
  // module constant (not config-derived), so no sanitizeForTerminal needed.
  if (agent.deprecated) {
    console.log(`    ${WARN} ${C.yellow}Deprecated — ${agent.deprecated}${C.reset}`);
  }
...
function printEnvScan(envFiles) {
  ...
  if (envFiles.length === 0) {
    ...
    for (const f of envFiles.slice(0, 10)) {
      console.log(`      ${C.dim}${sanitizeForTerminal(f)}${C.reset}`);
    }
 ...
 *   env: { ran, incomplete } — EXIT-05 (G-1623, D-20-06): a structural
 *     clone of `mcp` above, for the OTHER scan half. A caller that omits
 *     it (or passes a shapeless object) has NOT proven the `.env` scan
 *     ran, so it fails closed exactly like `mcp` does — see the WR-02
 ...
 *   - env ceiling (EXIT-05, G-1623, D-20-06): an incomplete `.env` scan
 *     (env.ran === false OR env.incomplete === true) — and equally a
 *     MISSING or shapeless `env` input (same WR-02 fail-closed shape as
 *     `mcp`) — fails closed at ceiling 2, independently of the MCP cap
 *     above: both can be active at once, each recording its OWN cap
 *     entry, because they are two different unfinished scans and neither
 *     one's incompleteness excuses the other.
 ...
 */
function computeSecurityLevel({ agentLevels = [], envFileCount = 0, mcp, env } = {}) {
  ...
  // EXIT-05 (G-1623, D-20-06): the SAME fail-closed shape, cloned for the
  // env half. A bare boolean (`envIncomplete: true`) cannot distinguish
  // "absent" from "false", so it cannot fail closed the way an object can
  // — `typeof env.ran === 'boolean'` is the same shape proof `mcp` uses.
  const e = (env && typeof env.ran === 'boolean')
    ? env
    : { ran: false, incomplete: true };
  ...
  // env-incomplete cap (EXIT-05, G-1623, D-20-06): mirrors mcp-incomplete
  // below exactly — same ceiling (2), same fail-closed-on-absence shape,
  // same "only fires when it reduces" guard. Placed immediately after the
  // env-files cap so the two `.env`-derived caps are adjacent; this
  // affects only DISPLAY ORDER in caps[]/levelCaps, never the Math.min
  // fold below — no test pins the order WITHIN caps[] (only the top-level
  // envelope key order is frozen, D-20-09).
  const envScanIncomplete = e.ran === false || e.incomplete === true;
  if (envScanIncomplete) {
    const ceiling = 2;
    if (base > ceiling) {
      caps.push({
        id: 'env-incomplete',
        cappedFrom: base,
        cappedTo: ceiling,
        reason: '.env scan could not complete — Level 3+ cannot be certified',
      });
    }
  }
  ...
  // F9: caps[] IS the record of fired ceilings — each cap.cappedTo is a
  // ceiling strictly below base (a ceiling only fires when it reduces),
  // so the min of the recorded cappedTo values equals
  // min(base, ...firedCeilings) with no parallel array to keep in sync.
  const level = caps.length ? Math.min(...caps.map((c) => c.cappedTo)) : base;
 ...
 */
/**
 * mcpGroupKey(finding) — the composite (agentId, scope, serverName) key that
 * printMcpScan's group Map is keyed on. The delimiter is a NUL, by design.
 *
 * (a) THE DELIMITER IS A REAL NUL AT RUNTIME AND MUST STAY ONE (D-05, locked).
 *     `agentId` is drawn from the fixed 10-item KNOWN_AGENT_IDS
 *     (lib/mcp/config-sources.js:19-22) behind a fail-closed dispatch guard
 *     (lib/scan-mcp.js:137-141), and `scope` from a fixed 4-value vocabulary —
 *     user | local | project | global (lib/mcp/config-sources.js:77-143).
 *     Both are NUL-free, so the key parses unambiguously and no collision is
 *     reachable. Only `serverName` is attacker-controlled, and a NUL cannot
 *     appear in it without appearing in the config file that produced it.
 *
 * (b) An earlier report that this key is INJECTABLE was WRONG, and is retracted.
 *     Changing this to a printable delimiter would INTRODUCE the flaw that was
 *     wrongly reported: a `serverName` containing that character would forge a
 *     group boundary and attribute findings to a different agent or scope in the
 *     operator's report. A future reader must not "simplify" this.
 *
 * (c) G-1573 changed the ENCODING only, never the delimiter. The escape below
 *     and a literal 0x00 byte are the SAME string value, so groupsByKey
 *     behaviour is byte-identical. What changes is that this file is no longer
 *     skipped by `grep -I`-class tools. Before the change they returned exit 1
 *     with NO output on it — indistinguishable from "no match" — leaving the one
 *     file that renders hostile MCP config data to the operator's terminal
 *     invisible to routine security greps, including the agent tooling that
 *     reviews this repo. tests/no-nul-source.test.js keeps every tracked and
 *     every shipped file NUL-free from here on.
 *
 * (d) The key is an internal Map key and is deliberately NOT passed through
 *     sanitizeForTerminal(). Sanitization happens at the two PRINT sites in
 *     printMcpScan; sanitizing the key instead would merge two distinct servers
 *     whose names differ only in the characters the sanitizer strips.
 */
function mcpGroupKey(finding) {
  return `${finding.agentId}\u0000${finding.scope}\u0000${finding.serverName}`;
}
