...
function printLevel(level, caps = []) {
  ...
  for (const cap of caps) {
    console.log(`  ${C.dim}Level capped at ${cap.cappedTo} (was ${cap.cappedFrom}): ${cap.reason}${C.reset}`);
  }
}
...
/**
 * printMcpAuditSection(envelope) — a COMPACT posture line for the main
 * scorecard (audit/install), sitting beside the .env block. This is
 * deliberately NOT the full per-server report (printMcpScan owns that);
 * it renders exactly one state, chosen by precedence:
 *
 ...
 * envelope.
 */
function printMcpAuditSection(envelope) {
