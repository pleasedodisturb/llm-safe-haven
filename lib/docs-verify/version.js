...
//
// F3 (Codex review, PR #105): the capture grammar is semver-shaped --
// `[\^~]?\d+(?:\.\d+)*(?:[-+][\w.-]*\w)?` -- rather than the old bare
// `[\w.+-]*` tail, which included a literal `.` in the character class
// and so greedily swallowed a trailing sentence period ("...@0.7.0. Then
// run it." captured "0.7.0."). The tightened grammar cannot end in `.`,
// `,`, `;`, or `:` (none of those are in either the digit-run segment or
// the required-trailing-`\w` prerelease/build segment), while still
// capturing a full prerelease+build suffix like `1.2.3-beta.1` and a
// leading `^`/`~` range operator.
const SELF_VERSION_PATTERNS = Object.freeze([
  // `<pkg>@<version>` -- npm package-manager pin shape.
  (escapedName) => new RegExp(`\\b${escapedName}@([\\^~]?\\d+(?:\\.\\d+)*(?:[-+][\\w.-]*\\w)?)`, 'g'),
