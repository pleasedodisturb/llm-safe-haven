 ...
 * sources.
 *
 * CHECK 7 DECISION (G-1672 AC3, D-04): the phase-21 first run reported
 * three unbound-claim `warn` rows -- CLAUDE.md:18 ("Per-agent hardening
 * guides (6 agents)"), README.md:183 ("8 detectors for MCP server
 * configs"), and docs/mcp-security.md:151 ("Two agents were evaluated and
 * later dropped"). The first two are bound here: each has a canonical
 * source and both numbers were already correct, so binding them turns two
 * `warn` rows into silent passes without changing a single number in
 * either document. The third is left unbound and is resolved by content
 * (22-05 rewords the sentence to name the agents instead of counting
 * them) -- Aider/Mistral are explicitly-excluded agents, not counted
 * anywhere this module can cite as a canonical source, so binding it would
 * require inventing one. The `warn` renderer itself is unchanged by this
 * decision -- no "unverified claims" header was introduced; an unbindable
 * claim in a scoped doc still surfaces as a `warn`, exactly as before.
 */
...
const CLAIM_REGISTRY = Object.freeze([
  ...
  {
    // G-1672 D-04. Deliberately NOT a reuse or widening of
    // 'hardening-guides-for-agents' above -- that pattern requires the
    // word "for" between "guides" and the count ("hardening guides for N
    // agents"). CLAUDE.md:18's real phrasing is "Per-agent hardening
    // guides (N agents)" -- no "for" -- which is exactly why it was
    // reported as an unbound `warn` rather than matching that entry.
    name: 'per-agent-hardening-guides-parenthesized',
    pattern: /Per-agent hardening guides \((\d+|[A-Za-z]+) agents\)/i,
    source: 'hardening-guides',
  },
  {
    // G-1672 D-04. README.md:183's real phrasing ("8 detectors for MCP
    // server configs"). The `detectors?` alternation tolerates the
    // singular spelling too -- a grammar generalisation of this ONE
    // phrase (so a count of exactly one, e.g. this module's own test
    // fixture, can be stated grammatically), never a widening to any
    // count sentence.
    name: 'detectors-for-mcp-server-configs',
    pattern: /(\d+|[A-Za-z]+) detectors? for MCP server configs/i,
    source: 'mcp-detectors',
  },
]);
