...
/**
 * Lists every `.js` file under `dir` (recursive). A directory that
 * genuinely does not exist (ENOENT) is treated as zero source files
 * there, never as a broken sweep -- not every root under inspection has
 * both a `hooks/` and a `lib/` top-level directory (e.g. this repo's own
 * pre-existing docs:verify fixture roots built for other checks, which
 * this check is also swept against via the real CLI's loadChecks()
 * registry). Any OTHER listing error (permission denied, etc.) still
 * throws -- only "no such directory" is benign.
 */
function listJsFiles(context, dir) {
  const listing = context.listFiles(dir, { ext: '.js', recursive: true });
  if (listing.error) {
    if (listing.error === 'ENOENT') return { files: [] };
    throw new Error(`identifiers: could not list ${dir}: ${listing.error}`);
  }
  return listing;
}
 ...
 * silent "clean") on a listFiles/readText error other than a missing
 * hooks/ or lib/ directory, so a broken sweep is never mistaken for a
 * clean one.
 */
function run(context) {
  const hooksListing = listJsFiles(context, 'hooks');
  const libListing = listJsFiles(context, 'lib');
