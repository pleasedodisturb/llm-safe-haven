...
/**
 * F2 (Codex review, PR #105): validates ONE finding record's shape, not a
 * check's overall return type -- that is `Array.isArray(result)`, already
 * checked separately in runAll() below. Array.isArray([{ severity: 'x' }])
 * is true, so a malformed record inside an otherwise well-shaped array
 * previously sailed straight past every gate and into tallySeverities(),
 * which silently ignores any severity that isn't exactly 'fail'/'warn' --
 * letting a broken check's output masquerade as zero findings.
 *
 * Grammar (matches every real check's own finding shape, verified against
 * all seven checks in lib/docs-verify/*.js): an object with severity in
 * {'fail','warn'}; check, file, message all non-empty strings; line either
 * absent or a non-negative integer (checks always populate it as a number
 * today, but the grammar permits omitting it -- a bare presence/type gate,
 * not a today's-callers gate).
 */
function isValidFinding(record) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return false;
  if (!VALID_SEVERITIES.has(record.severity)) return false;
  if (typeof record.check !== 'string' || record.check === '') return false;
  if (typeof record.file !== 'string' || record.file === '') return false;
  if (typeof record.message !== 'string' || record.message === '') return false;
  if (record.line !== undefined && !(Number.isInteger(record.line) && record.line >= 0)) return false;
  return true;
}

/**
 * Names WHICH grammar rule the first invalid record in `result` violates,
 * for the incomplete-reason string -- "index/what" per the finding's own
 * spec, so a contributor debugging a broken check sees more than a bare
 * index.
 */
function describeInvalidFinding(record) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return 'not-an-object';
  if (!VALID_SEVERITIES.has(record.severity)) return `invalid-severity:${JSON.stringify(record.severity)}`;
  if (typeof record.check !== 'string' || record.check === '') return 'invalid-check';
  if (typeof record.file !== 'string' || record.file === '') return 'invalid-file';
  if (typeof record.message !== 'string' || record.message === '') return 'invalid-message';
  if (record.line !== undefined && !(Number.isInteger(record.line) && record.line >= 0)) return 'invalid-line';
  return 'unknown';
}
...
function runAll(context, checks, checkErrors) {
  ...
  for (const check of Array.isArray(effectiveChecks) ? effectiveChecks : []) {
    ...
    // F2: every record inside the array must ALSO be well-formed -- a
    // well-shaped array containing one malformed record must be rejected
    // in full (mirrors check-returned-non-array's all-or-nothing
    // handling), never partially accepted, since a check that emits N-1
    // good records and 1 broken one is exactly as untrustworthy as one
    // that returns a bare object.
    const invalidIndex = result.findIndex((r) => !isValidFinding(r));
    if (invalidIndex !== -1) {
      incomplete.push({
        check: check.id,
        reason: `check-returned-malformed-finding: ${invalidIndex}/${describeInvalidFinding(result[invalidIndex])}`,
      });
      continue;
    }
...
module.exports = { loadChecks, runAll, tallySeverities, computeExit, formatReport, EXIT, isValidFinding };
