...
/**
 * isPlaceholderSegment(segment) -- exactly three disjoint shapes: an
 * angle-bracketed name, a wildcard token, or a date-shaped token
 * (delegated to isDateShapedSegment).
 */
...
function buildSourceEvidence(context) {
  ...
  return { compositionTuples, bareLiterals };
}
