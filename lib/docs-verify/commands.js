 ...
 *
 * NO FIFTH PLACEHOLDER SHAPE (G-1672 D-03, item A): a "descriptive example
 * name" heuristic (a path segment beginning `your-`/`my-`/`example-`, or
 * similar) was considered and deliberately NOT added here. That shape is a
 * blocklist wearing a grammar's clothing -- it enumerates prefixes an
 * author might choose for a placeholder, rather than deriving placeholder-
 * ness from the token's own structure the way the four shapes above do.
 * This project already paid five iterations for exactly that anti-pattern
 * on the POSIX-class validator (feedback_validators_as_grammars) and is
 * not paying it again here. The three example paths that motivated the
 * idea (`lib/agents/your-agent.js` in CLAUDE.md and README.md,
 * `manifests/waves/your-new-wave.json` in docs/wave-spec.md) are instead
 * REWORDED to the angle-bracket shape this grammar already exempts
 * (`lib/agents/<your-agent>.js`, `manifests/waves/<your-new-wave>.json`)
 * -- resolved by content, in 22-04/22-08, never by widening this module.
 *
 * HONEST SCOPE, two further stated precision limits (G-1672):
 *
 * 1. Deleted and foreign files (D-05). A documented token naming a module
 *    this repository deliberately deleted, or a file that belongs to a
 *    different repository, is indistinguishable by a presence test from a
 *    token naming a file that should exist and does not -- both are
 *    reported as `fail`. The resolution is content: reword the sentence
 *    until the token is no longer shaped as a claim about this
 *    repository's own tree. Never an allow-list, and never a created
 *    placeholder file that would make the false claim technically true.
 *    Real corpus examples: `lib/traverse/git-ignore.js` (deleted
 *    2026-08-07, Phase 17 D-13, mentioned in docs/supply-chain-defense.md
 *    and docs/wave-spec.md as history) and `.github/copilot-instructions.md`
 *    (docs/threat-model.md:253's example of ANOTHER repository's
 *    convention, not a claim about this one).
 *
 * 2. Foreign agent-home files (D-02, implemented by
 *    isRepoOwnedAgentHomeToken/deriveAgentHomeOwnedSegments below). An
 *    agent-home token is graded only where this repository composes a
 *    segment by that name immediately under the agent home (derived from
 *    hooks/**+lib/** path.join/path.resolve evidence -- never a
 *    hand-maintained list of "their" paths). A documented path under
 *    another tool's home directory that this repository never writes --
 *    `~/.claude/config.json`, `~/.claude/.credentials.json`,
 *    `~/.claude/profiles/work/settings.json`, all genuinely Claude Code's
 *    own files -- is out of scope and produces no finding. The accepted
 *    consequence, stated by name: an INVENTED path under the agent home
 *    whose first segment this repository happens not to compose is also no
 *    longer caught, even though it is a real defect. The live example that
 *    motivated this note is docs/testing.md:863's `~/.claude/session-logs`
 *    -- an invented path with no first-segment match in the composed set.
 *    It is fixed by content in 22-03, against the real Claude Code
 *    transcript location, in this same phase -- the guard going quiet on
 *    that row is never accepted as the fix, only as a stated, disclosed
 *    limit of what a presence/composition test over source literals can
 *    prove.
 *
 *    G-1672 CR-01: a stem match (e.g. 'settings' standing in for the
 *    composed 'settings.json') is honoured ONLY when the anchor segment IS
 *    the documented token's own final segment -- i.e. the token names the
 *    composed file directly. It is never honoured when the anchor merely
 *    starts a longer, unrelated path (`~/.claude/settings/<profile>.json`
 *    stays out of scope even though 'settings' is a real derived stem --
 *    that directory is a different, foreign Claude Code path this
 *    repository never composes). deriveAgentHomeOwnedSegments therefore
 *    returns two SEPARATE sets -- exact composed segments and their
 *    stems -- rather than merging both into one set, which is what let a
 *    stem satisfy an exact-match check regardless of the anchor's position.
 */
...
/**
 * deriveAgentHomeOwnedSegments(compositionTuples) -- D-02: the segments
 * this repository composes IMMEDIATELY under an agent-home marker, derived
 * from compositionTuples (never enumerated by hand). For every tuple,
 * wherever AGENT_HOME_SEGMENT ('.claude') occurs, the segment that follows
 * it is added to `agentHomeOwnedSegments` (the EXACT composed literal) and
 * its stem (segmentStem) is added SEPARATELY to `agentHomeOwnedStems`.
 ...
 * (`('.claude', 'settings.json')`) together derive the real-repo exact set
 * `{audit, hooks, settings.json}` and stem set `{audit, hooks, settings}`
 * -- confirmed at planning time (`/usr/bin/grep -rn "'\.claude'" hooks lib`)
 * and re-derived here at every run, never pinned as a literal list.
 *
 * G-1672 CR-01: the two sets are returned SEPARATELY, never merged into
 * one. isRepoOwnedAgentHomeToken (below) consults the stem set only when
 * the anchor is the documented token's own final segment -- merging both
 * into a single set (the pre-fix shape) let a bare stem satisfy an
 * exact-match check regardless of the anchor's position in the token,
 * wrongly widening scope to an unrelated, deeper subtree
 * (`~/.claude/settings/<profile>.json` via the 'settings' stem of the
 * composed 'settings.json').
 */
function deriveAgentHomeOwnedSegments(compositionTuples) {
  const agentHomeOwnedSegments = new Set();
  const agentHomeOwnedStems = new Set();
  for (const tuple of compositionTuples) {
    for (let i = 0; i < tuple.length - 1; i += 1) {
      if (tuple[i] === AGENT_HOME_SEGMENT) {
        const next = tuple[i + 1];
        agentHomeOwnedSegments.add(next);
        agentHomeOwnedStems.add(segmentStem(next));
      }
    }
  }
  return { agentHomeOwnedSegments, agentHomeOwnedStems };
}
 ...
 *   - agentHomeOwnedSegments / agentHomeOwnedStems: derived from
 *     compositionTuples via deriveAgentHomeOwnedSegments (D-02) -- the
 *     exact segments this repository genuinely composes immediately under
 *     an agent home, and their stems (kept as a SEPARATE set -- G-1672
 *     CR-01), feeding the in-scope predicate below.
 */
function buildSourceEvidence(context) {
  ...
  const { agentHomeOwnedSegments, agentHomeOwnedStems } = deriveAgentHomeOwnedSegments(compositionTuples);

  return { compositionTuples, bareLiterals, agentHomeOwnedSegments, agentHomeOwnedStems };
}
 ...
 * grading iff EITHER (a) that segment equals a segment this repository
 * composes under the same agent home (evidence.agentHomeOwnedSegments,
 * exact match -- valid at ANY position), OR (b) that segment's stem
 * (segmentStem) equals a stem this repository composes
 * (evidence.agentHomeOwnedStems) AND the anchor is the token's OWN FINAL
 * segment. A token whose anchor segment fails both tests is not this
 * repository's claim to make and produces NO finding at any tier -- not a
 * pass-with-weak-evidence, which would be a different and quieter lie.
 *
 * G-1672 CR-01: the final-segment condition on the stem match is load
 * bearing. Without it, a directory-shaped token that merely shares its
 * FIRST segment's name with an unrelated composed file's stem (e.g.
 * `~/.claude/settings/<profile>.json`, first segment 'settings' -- the
 * stem of the composed 'settings.json' -- but naming a different, foreign
 * subtree this repository never writes) is wrongly pulled into scope and
 * then fails grading on a segment ('settings' itself, or whatever follows
 * it) that was never a literal in source. A stem is only a legitimate
 * stand-in for the composed file's bare name when the documented token
 * names that file directly.
 *
...
function isRepoOwnedAgentHomeToken(segments, evidence) {
  const anchorIdx = segments.findIndex((s) => !isPlaceholderSegment(s));
  if (anchorIdx === -1) return true;
  const anchor = segments[anchorIdx];
  if (evidence.agentHomeOwnedSegments.has(anchor)) return true;
  const isFinalSegment = anchorIdx === segments.length - 1;
  return isFinalSegment && evidence.agentHomeOwnedStems.has(segmentStem(anchor));
}
