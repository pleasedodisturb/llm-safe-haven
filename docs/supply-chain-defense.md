...
| Mini — ChainDrop ("Self-Propagating Worm") | Aug 4, 2026 | `keyv` maintainer GitHub account → `keyv` (155M/wk), `flat-cache` (150M/wk), `file-entry-cache` (148M/wk), `cacheable`/`cacheable-request`/`cache-manager` + `@keyv/*` + `@cacheable/*` family, self-propagated to 400–868+ packages | 443–1,381 versions across ~400–1,300 packages, ~2B monthly downloads, ~1.5h malicious window | **Poisoned releases carry VALID GitHub-Actions/SLSA provenance** (push-to-main → auto-release) — `npm audit signatures` PASSES them, defeating provenance/signature scanners; Bun-staged obfuscated worm (`setup.mjs` → `Math_Symbol.js`) harvests npm/GitHub/AWS/K8s/Vault creds; **plants Claude Code + VS Code hooks** as persistence; Ethereum smart-contract dead-drop C2; arms a token-revocation watcher that detonates on credential rotation |

[Source: Snyk, Wiz, StepSecurity, Akamai, SafeDep, Microsoft, Kodem, Socket, Aikido, Chainguard — see Sources section.]
...
#### Wave J — ChainDrop: Valid-Provenance Worm Hits keyv/cacheable (August 4, 2026)

On **August 4, 2026 at ~09:00 UTC**, an attacker who had compromised the GitHub account of the `keyv`/`cacheable` maintainer pushed malicious commits directly to `main` and cut a release. The first poisoned artifact — `keyv@6.0.0` — published at 09:35 UTC through the project's own legitimate GitHub Actions trusted-publishing workflow. The worm then self-propagated across the `@keyv/*`, `@cacheable/*`, `flat-cache`, `file-entry-cache`, `cacheable-request`, and `cache-manager` family and onward, reaching **400–868+ packages / 443–1,381 versions** (counts vary by vendor and snapshot time) with a combined **~2 billion monthly downloads**, all inside a **~1.5-hour** window before npm began mass-unpublishing at ~10:39 UTC.

**Why this one is different — provenance is not a defense.** Because the compromised account pushed to `main` and the *untouched, legitimate* release workflow built and signed the result, every poisoned release carries **valid GitHub-Actions / SLSA provenance**. `npm audit signatures` returns green. The canonical framing from post-incident analysis: **provenance proves lineage, not authorization** — it proves *which commit was built*, not that *the commit was authorized*. Any defense that trusts an attestation's presence is blind to this wave. Detection must be IOC- and behaviour-based.

**Execution chain.** A hidden `"preinstall": "node setup.mjs"` runs on `npm install`; `setup.mjs` (a small loader, always exits 0 to avoid breaking the install) downloads a **Bun** runtime into `$TMPDIR/bun-dl-*` and executes a ~728 KB obfuscated stage-2 harvester shipped as `Math_Symbol.js` (later variants: `math_init.js`). It steals npm, GitHub, AWS, Kubernetes, and Vault credentials, self-republishes using any stolen npm trusted-publishing tokens, and retrieves its C2 domain from an **Ethereum smart contract** (`0xE1f2395ee43e45A1556EC6438a88c31B83493103`) — a takedown-resistant dead-drop — with `npm-cache[.]com` (`104.21.35[.]216/router`) observed as an active C2.

**Persistence — the primary re-execution path, no install required.** Like Waves A/C/I, ChainDrop injects a `SessionStart` hook into `.claude/settings.json` and a `folderOpen` task (named **"Environment Setup"**) into `.vscode/tasks.json`. Simply opening the repo in an AI-enabled editor, or starting a Claude Code session, re-detonates the payload — **`--ignore-scripts` and cache purges do nothing** against this path. It also installs a **`gh-token-monitor`** watcher (at `~/.local/bin/gh-token-monitor.sh` plus a systemd unit / macOS LaunchAgent) — see the remediation-order warning below.

**Scale of the family.** `keyv` (~155M weekly downloads), `flat-cache` (~150M), `file-entry-cache` (~148M) are ubiquitous transitive dependencies of ESLint — their *presence* in your tree is expected and is **not** an indicator. Only a **poisoned version** is: `keyv@6.0.0`, `flat-cache@6.1.24`, `file-entry-cache@11.1.6`, `cacheable-request@13.0.20`, `cacheable@2.5.1`, `cache-manager@7.2.10`, `@cacheable/*@2.x`, and every `@keyv/*@6.0.0`. Last-known-good and the full poisoned map are bundled in [`manifests/chaindrop-poisoned-versions.json`](../manifests/chaindrop-poisoned-versions.json).

**Detection:** `npx llm-safe-haven scan --supply-chain` runs the ChainDrop IOC scanner (`scripts/scan-chaindrop-aug2026.sh`) by default — file markers, the preinstall marker, poisoned-version cross-check against the bundled manifest, the install-window heuristic (versions published ≥ 2026-08-04T09:30:00Z), agent-config persistence hooks, network/behavioural markers, and an optional GitHub dead-drop audit.

> **⚠️ Remediation order matters (ChainDrop-specific).** The `gh-token-monitor` watcher is designed to **detonate a destructive payload when a stolen token is revoked**. If you find indicators: (1) capture evidence, (2) **remove the watcher and its systemd unit / LaunchAgent *first*,** (3) *then* rotate credentials from a clean machine. Rotating first can trigger the payload. Also purge the npm/bun cache, not just `node_modules` — poisoned versions were unpublished *after* being installed.

Source: [Microsoft Security — ChainDrop: Anatomy of a Self-Propagating Worm](https://www.microsoft.com/en-us/security/blog/2026/08/04/chaindrop-supply-chain-compromise-anatomy-self-propagating-worm/) | [StepSecurity — ChainDrop npm Worm](https://www.stepsecurity.io/blog/chaindrop-npm-worm) | [Kodem — keyv IOCs & First-Hour Runbook](https://www.kodemsecurity.com/resources/keyv-supply-chain-attack-shai-hulud-npm-worm-affected-versions-iocs-and-first-hour-response-runbook) | [Wiz — keyv and cacheable npm Supply Chain Attack](https://www.wiz.io/blog/keyv-and-cacheable-npm-supply-chain-attack) | [Chainguard](https://www.chainguard.dev/unchained/the-keyv-and-cacheable-npm-supply-chain-attack-inside-the-mini-shai-hulud-campaign) | [BleepingComputer](https://www.bleepingcomputer.com/news/security/massive-chaindrop-npm-supply-chain-attack-infects-hundreds-of-packages/) | IOC data cross-verified against [OSV MAL-2026-11524](https://osv.dev/vulnerability/MAL-2026-11524) and [ossf/malicious-packages](https://github.com/ossf/malicious-packages)
...
14. **Wave J (ChainDrop, Aug 4, 2026): Run `npx llm-safe-haven scan --supply-chain`** — it now defaults to the ChainDrop scanner. Treat any `keyv@6.0.0`, `flat-cache@6.1.24`, `file-entry-cache@11.1.6`, `cacheable@2.5.1`, `cacheable-request@13.0.20`, `cache-manager@7.2.10`, `@cacheable/*@2.x`, or `@keyv/*@6.0.0` in your lockfiles as compromised (pin to last-known-good in [`manifests/chaindrop-poisoned-versions.json`](../manifests/chaindrop-poisoned-versions.json)). **`npm audit signatures` will NOT flag these** — the poisoned releases carry valid provenance. If you find indicators, **remove the `gh-token-monitor` watcher (`~/.local/bin/gh-token-monitor.sh` + its systemd unit / LaunchAgent) BEFORE rotating any credentials** — revoking a stolen token detonates the watcher's payload. Then rotate from a clean machine and purge the npm/bun cache.
...
**`--ignore-scripts` is necessary but not sufficient — treat it as one layer, not the answer.** The 2026 wave series repeatedly routed *around* it:

- **Phantom Gyp (Wave E)** ships a `binding.gyp`; npm's implicit `node-gyp rebuild` runs it with no lifecycle script at all.
- **Import-time payloads** (AsyncAPI, the Hades PyPI waves) execute on first `require()`/`import` — install flags are irrelevant.
- **ChainDrop (Wave J)** persists in `.claude/settings.json` and `.vscode/tasks.json` and re-detonates on *folder-open / agent-session-start* — **with no `npm install` at all**.

The ecosystem's actual structural answer shipped in mid-2026 and is what you should adopt:

1. **Upgrade to npm v12 (or pnpm v11)** so install scripts — *including implicit `binding.gyp` builds* (the direct Phantom Gyp fix) — are **default-deny with a per-package allowlist** (`npm approve-scripts --allow-scripts-pending`, committed to source). This is stronger than a blanket `ignore-scripts` because it still lets *your* vetted native addons build.
2. **Turn on an install cooldown / minimum-release-age** (npm `min-release-age`, pnpm `minimumReleaseAge` — default 1 day in v11 — Yarn/Bun equivalents). Set **3–7 days**. ChainDrop's entire malicious window was ~1.5 hours; even a 1-day cooldown would have fully covered it, since npm had unpublished the poisoned versions long before the delay elapsed.
3. **Do not treat provenance/SLSA attestation as a trust signal by itself.** All three major 2026 waves (TanStack, Miasma, ChainDrop) shipped *validly attested* malware. **Lineage ≠ authorization** — verify policy (expected repo, reviewed release), not merely that an attestation exists.

`--ignore-scripts` still belongs in CI and blocks the classic `preinstall`/`postinstall` vector (including ChainDrop's `node setup.mjs` at *install* time) — keep it. Just don't mistake it for coverage of the install-independent execution paths above; a file-marker + config-persistence scan (`npx llm-safe-haven scan --supply-chain`) is the gate for those. ([StepSecurity — ChainDrop](https://www.stepsecurity.io/blog/chaindrop-npm-worm) · [npm v12 changelog](https://github.blog/changelog/2026-06-09-upcoming-breaking-changes-for-npm-v12/))
