...
| Mini — Miasma (Leo Platform / Go) | June 24, 2026 | 20+ LeoPlatform/RStreams npm packages via compromised maintainer `czirker` (SDK/CLI/AWS/cron/logging/connector/serverless) + `hexo-deployer-wrangler`/`hexo-shoka-swiper`/`prism-silq` via `llxlr`; related Verana Blockchain Go module | 20+ packages, ~6-second push window | **First Go-ecosystem element** of the lineage; reuses Phantom Gyp `binding.gyp` install-time execution; **Bun-staged payload** (`/tmp/p.js`, Bun v1.3.13) to evade Node-focused EDR; harvests CI/CD + cloud + AI-agent config secrets to a GitHub dead-drop |
| Mini — Miasma (@immobiliarelabs Backstage) | June 26, 2026 | 4 `@immobiliarelabs/backstage-plugin-*` npm packages via `codfish/semantic-release-action` tag-hijacking (June 24) | ~600K monthly downloads | Phantom Gyp `binding.gyp`; Bun-staged payload; AES-128-GCM GitHub dead-drop; **plants AI coding assistant persistence hooks** into `.claude/settings.json` and `.vscode/tasks.json` |
| Mini — Miasma "M-RED-TEAM" (AsyncAPI) | July 14, 2026 | 4 `@asyncapi/*` packages (`specs`, `generator`, `generator-components`, `generator-helpers`) via a "pwn request" (`pull_request_target`) that stole the `asyncapi-bot` GitHub PAT | 5 versions, 2.25M–3M+ combined weekly downloads | **First import-time-execution payload on npm** (previously PyPI-only, Hades); **PAT theft feeding OIDC trusted-publishing abuse** (valid SLSA Build Level 3 provenance, passes `npm audit signatures`); decentralized IPFS/Nostr/Ethereum/BitTorrent C2 fallback chain; still plants `.claude/settings.json` + `.vscode/tasks.json` persistence |

[Source: Snyk, Wiz, StepSecurity, Akamai, SafeDep, Microsoft, Datadog — see Sources section.]
...
#### Wave H — Miasma Returns: Leo Platform / RStreams + First Go-Ecosystem Element (June 24, 2026)

Two weeks after Wave G, Socket Threat Research tracked a new burst in the Mini Shai-Hulud / Miasma / Hades family. On June 24, 2026 a compromised npm maintainer account (`czirker`) pushed trojanized versions of **20+ LeoPlatform/RStreams packages** (SDK, CLI, AWS, cron, logging, connector, and serverless packages used in data-pipeline and cloud-integration workflows) within a roughly **6-second window**; npm user `llxlr` separately published `hexo-deployer-wrangler`, `hexo-shoka-swiper`, and `prism-silq`.

**Execution:** Reuses the **Phantom Gyp `binding.gyp`** install-time technique — code runs during `npm install` via node-gyp, so `--ignore-scripts` and `package.json` lifecycle-hook auditing do not protect you — then **stages the toolkit to `/tmp/p.js` and runs it under the Bun runtime (v1.3.13) rather than Node** to evade Node-focused EDR.

**Collection:** `.env` files, npm/PyPI/GitHub/Slack/Twilio tokens, SSH keys, Docker auth, Kubernetes configs, AWS/Azure/GCP credentials, Vault data, shell history, CI secrets, and **IDE / AI-agent configuration paths**; exfiltration is encrypted and routed through the GitHub API to a dead-drop.

**First Go-ecosystem element:** A related compromise of the **Verana Blockchain Go project** staged payloads inside a `.claude/` folder, with a VS Code folder-open task invoking `node .claude/setup.mjs` — the first time this lineage has reached beyond npm/PyPI into Go module tooling.

Source: [Socket.dev — Miasma Mini Shai-Hulud Hits LeoPlatform npm Packages and the Go Ecosystem](https://socket.dev/blog/miasma-mini-shai-hulud-hits-leoplatform-npm-packages-go-ecosystem) | [The Register — Miasma campaign poisons 20-plus npm packages (June 26, 2026)](https://www.theregister.com/security/2026/06/26/miasma-campaign-poisons-20-plus-npm-packages-hunts-for-developer-secrets/5262886) | [Sonatype — Miasma Returns: Leo Platform Compromise in npm](https://www.sonatype.com/blog/miasma-returns-leo-platform-compromise-in-npm) (HTTP 403 — bot-protection pattern; search-confirmed live)

#### Wave I — Miasma Returns: @immobiliarelabs Backstage Plugins (June 26, 2026)

Two days after Wave H, the Miasma family struck again. On **June 26, 2026**, four `@immobiliarelabs/backstage-plugin-*` npm packages were backdoored via a **tag-hijacking attack on the `codfish/semantic-release-action` GitHub Action** (a Backstage CI dependency), triggering a malicious `setup.sh` in the plugin build pipeline. The hijacked action published trojanized versions of the four plugins to npm.

**Execution:** Reuses the **Phantom Gyp `binding.gyp`** install-time technique (bypasses `--ignore-scripts`). Payload is staged as a Bun-executed script and encrypted using **AES-128-GCM**, with the key fetched from a GitHub API dead-drop to frustrate static analysis.

**Collection and persistence:** Harvests npm/GitHub/Slack/Twilio tokens, SSH keys, Docker auth, Kubernetes configs, AWS/Azure/GCP credentials, CI secrets, shell history, and `.env` files. Specifically **plants persistence hooks into AI coding assistant configurations** — injects a `SessionStart` hook into `.claude/settings.json` and a `folderOpen` task into `.vscode/tasks.json` — ensuring re-execution on every agent session start in affected repositories.

**Scale:** The four `@immobiliarelabs/backstage-plugin-*` packages had a combined **~600K monthly downloads**.

Source: [StepSecurity — @immobiliarelabs Backstage Plugin Supply Chain Attack](https://www.stepsecurity.io/blog/immobiliarelabs-backstage-plugin-supply-chain-attack) (HTTP 403 — bot-protection pattern; search-confirmed live) | [Socket.dev — Miasma Wave I: Backstage Plugin Compromise](https://socket.dev/blog/miasma-wave-i-backstage-plugin-compromise) (HTTP 403 — bot-protection pattern; search-confirmed live)

#### Wave J — Miasma "M-RED-TEAM": AsyncAPI npm Compromise (July 14, 2026)

Nineteen days after Wave I, the same lineage struck again — this time introducing a technique this campaign hadn't used before: **import-time execution on npm** (previously seen only on PyPI, in the Hades waves).

**Attack chain:** The attacker opened ~36–37 decoy pull requests (fake charity-donation-page spam) against the AsyncAPI `generator` repo to bury one real attack PR (#2155), which exploited a **"pwn request"** — a workflow triggered by `pull_request_target` that checked out untrusted PR code inside the base repo's security context, leaking the `asyncapi-bot` GitHub PAT (and, per Datadog, Netlify credentials). The attacker then pushed malicious commits directly to unprotected pre-production branches (`next`, `schema`) as `asyncapi-bot`, triggering the *legitimate* release workflows. Those workflows published through **npm trusted publishing via GitHub OIDC**, so the resulting malicious versions carry **valid SLSA Build Level 3 provenance and pass `npm audit signatures`** — the same "trusted publishing doesn't guarantee trust" lesson as the TanStack (Wave B) and SAP CAP (Wave A) waves, now combined with PAT theft as the initial-access vector.

**Affected packages/versions:** `@asyncapi/specs` 6.11.2-alpha.1 / 6.11.2, `@asyncapi/generator` 3.3.1, `@asyncapi/generator-components` 0.7.1, `@asyncapi/generator-helpers` 1.1.1 — 5 versions across 4 packages, combined weekly downloads reported between 2.25M (Microsoft) and 3M+ (Datadog).

**Novel technique — import-time execution on npm:** Rather than a `preinstall`/`postinstall` hook or a `binding.gyp` build step, the malicious loader is embedded in normal runtime code paths — `@asyncapi/specs` → `index.js`, `generator` → `lib/templates/config/validator.js`, `generator-helpers` → `src/utils.js`, `generator-components` → `lib/utils/ErrorHandling.js` — and fires the moment the module is `require`/`import`ed, not at install time. This is the first time this lineage has ported PyPI's Hades import-time technique to npm; `--ignore-scripts` and lifecycle-hook auditing catch neither this nor the earlier `binding.gyp` vector.

**Second-stage delivery and C2:** The loader spawns a detached process that pulls an ~8.2 MB encrypted second stage from **IPFS** (self-identifying internally as `M-RED-TEAM v6.4`, build tag `miasma-train-p1`), with fallback C2 channels over Nostr, Ethereum (command validation via a smart contract), BitTorrent DHT, and libp2p — a decentralized, multi-protocol C2 resilience layer not seen in earlier waves (which relied on GitHub dead-drops or fixed C2 IPs).

**IOCs:** C2 `85.137.53[.]71` (ports 8080/8081/8091); IPFS CIDs `Qmet4fhsAaWMBUxNDfREHwgiyDeSWy4YSYs9wiKUW5jGyf` (specs stage) and `QmQobZSp1wRPrpSEQ56qnyq7ecZh5Bg5k1fnjt4SUwwHb9` (generator-family stage); Ethereum command-validation contract `0x12c37A86a0Ed0beBe5d1d6a43E42f07860eAc710`; attacker GitHub account `elzotebo999` (deleted); exfil paste channel `rentry.co/elzotebo`; Windows persistence `HKCU\...\Run\miasma-monitor`; Linux systemd user unit `miasma-monitor.service`; macOS shell-RC injection; drop paths `%LOCALAPPDATA%\NodeJS\sync.js` (Win) / `~/.local/share/NodeJS/sync.js` (Linux) / `~/Library/Application Support/NodeJS/sync.js` (macOS); lock file `~/.config/.miasma/run/node.lock`; local artifact cache `~/.cache/.sys_cache/.diag.enc`; publisher identity string `npm-oidc-no-reply@github[.]com`; mDNS service advertisement `_miasma._tcp`. Also modifies `/home/runner/work/_actions/actions/checkout/v5/dist/index.js` on the CI runner to retain post-run access.

**Attribution nuance:** The payload brands itself `M-RED-TEAM`/`miasma-train-p1` and shares hosting infrastructure (AS43641/VSYS-AMS, Netherlands) with prior confirmed TeamPCP operations, but since the worm source was published publicly on BreachForums in May, running this campaign no longer requires TeamPCP insider access. No vendor makes a definitive TeamPCP/UNC6780 attribution — treat it the same way this guide already treats Hades: Miasma/Shai-Hulud lineage, attribution unconfirmed.

**Detection in this toolkit:** `scripts/scan-miasma-june2026.sh` now also flags the Wave J package names, the `miasma-monitor` persistence artifacts, and the `M-RED-TEAM` / `elzotebo999` marker strings.

Source: [Microsoft Security Blog — Unpacking the AsyncAPI npm supply chain compromise](https://www.microsoft.com/en-us/security/blog/2026/07/15/unpacking-asyncapi-npm-supply-chain-compromise-import-time-payload-delivery/) (HTTP 200 verified) | [Datadog Security Labs — Compromised AsyncAPI npm packages](https://securitylabs.datadoghq.com/articles/compromised-asyncapi-npm-packages/) (HTTP 200 verified) | [Socket.dev — Compromised npm Packages in the AsyncAPI Namespace](https://socket.dev/blog/asyncapi-supply-chain-attack) | [StepSecurity — Compromised next branch pushes malicious AsyncAPI packages](https://www.stepsecurity.io/blog/compromised-next-branch-pushes-malicious-asyncapi-generator-generator-helpers-and-generator-components-to-npm) | [Wiz — M-Red-Team: AsyncAPI Supply Chain Compromise](https://www.wiz.io/blog/m-red-team-asyncapi-supply-chain-compromise-via-github-actions) | [The Hacker News — Compromised AsyncAPI npm Packages Deliver Multi-Stage Botnet Malware](https://thehackernews.com/2026/07/compromised-asyncapi-npm-packages.html) (all four HTTP 403 — bot-protection pattern; search-confirmed live)

#### Other npm incidents this week — not part of the Shai-Hulud/Miasma lineage

Two more npm compromises landed the same week; neither is attributed to TeamPCP/Miasma, so don't fold them into the wave table above:

- **Jscrambler (July 11, 2026):** stolen npm publishing credentials pushed malicious versions of `jscrambler` (8.14.0, 8.16.0, 8.17.0, 8.18.0, 8.20.0), `jscrambler-webpack-plugin` (8.6.2), `gulp-jscrambler` (8.6.2), `grunt-jscrambler` (8.5.2), and `jscrambler-metro-plugin` (9.0.2). Payload: a Rust-based infostealer targeting developer workstations, CI/CD, cloud credentials, crypto wallets, and AI coding assistant configs — the same target profile as Shai-Hulud, but no vendor links it to that lineage. Detected within minutes; Jscrambler deprecated the affected versions. Source: [The Hacker News — Compromised jscrambler 8.14.0 npm Release](https://thehackernews.com/2026/07/compromised-jscrambler-8140-npm-release.html) (HTTP 403 — bot-protection pattern; search-confirmed live via BleepingComputer, Socket.dev, TechNadu).
- **Injective Labs SDK (July 8, 2026):** `@injectivelabs/sdk-ts@1.20.21` and 17 dependent `@injectivelabs/*` packages backdoored via a compromised trusted-maintainer account, live under an hour. Crypto wallet-key/mnemonic stealer disguised as fake telemetry; no funds reported lost. Source: [The Hacker News — Injective Labs GitHub Compromise](https://thehackernews.com/2026/07/injective-labs-github-compromise-pushes.html) (HTTP 403 — bot-protection pattern; search-confirmed live via BleepingComputer, StepSecurity).
...
12. **Wave H (Leo Platform / Miasma, June 24, 2026): If you depend on LeoPlatform/RStreams npm packages** (data-pipeline / serverless SDKs), run `npm ls` against those dependencies and treat any version published June 24, 2026 onward as compromised — the payload executes at install via `binding.gyp`, so `--ignore-scripts` does **not** protect you. Also check the Bun-staged IOC with `find /tmp -maxdepth 1 -name "p*.js"` (already covered by `scripts/scan-shai-hulud-may2026.sh`). If you cloned the Verana Blockchain Go project, inspect `.claude/` for an unexpected `setup.mjs` and `.vscode/tasks.json` for a `"runOn": "folderOpen"` task. Rotate all CI/CD and cloud credentials reachable from any affected host.
13. **Wave I (@immobiliarelabs Backstage, June 26, 2026): If you use any `@immobiliarelabs/backstage-plugin-*` packages**, treat any version published June 26, 2026 as compromised. The payload executes at install via `binding.gyp` (`--ignore-scripts` does **not** protect you). Additionally, check `.claude/settings.json` for injected `SessionStart` hooks and `.vscode/tasks.json` for injected `"runOn": "folderOpen"` tasks you did not add — the payload plants AI coding assistant persistence hooks that re-execute on every agent session start.
14. **Wave J (Miasma/M-RED-TEAM, AsyncAPI, July 14, 2026): If you depend on `@asyncapi/specs`, `@asyncapi/generator`, `@asyncapi/generator-components`, or `@asyncapi/generator-helpers`**, run `npm ls @asyncapi/specs @asyncapi/generator @asyncapi/generator-components @asyncapi/generator-helpers` across your projects and check installed versions against `6.11.2`/`6.11.2-alpha.1` (specs), `3.3.1` (generator), `0.7.1` (generator-components), `1.1.1` (generator-helpers). Any match means the host **imported** the malicious code, since this payload fires at `require`/`import` time, not install time — merely having it in `node_modules` unused is lower risk, but any process that actually loaded the module should be treated as compromised. Check for outbound connections to `85.137.53[.]71` (ports 8080/8081/8091) and for the persistence artifacts: Windows `HKCU\...\Run\miasma-monitor`, Linux `~/.config/systemd/user/miasma-monitor.service` (also check `systemctl --user status miasma-monitor`), macOS shell-RC injections, and the lock file `~/.config/.miasma/run/node.lock`. As with prior waves, audit `.claude/settings.json` for an unexpected `SessionStart` hook and `.vscode/tasks.json` for an unexpected `"runOn": "folderOpen"` task. `npm audit signatures` and SLSA provenance checks will **not** catch this — the malicious versions carry valid Build Level 3 provenance because they were published through the legitimate trusted-publishing pipeline. If you maintain a GitHub Actions workflow using `pull_request_target` that checks out PR code, audit it now — this "pwn request" pattern is the root cause and is now confirmed exploited twice in this lineage (SAP CAP's Wave A, AsyncAPI's Wave J).
...
2. **Check for persistence** (fastest: `npx llm-safe-haven scan --supply-chain` runs the bundled June Miasma scanner network-free on macOS/Linux. Or run the scripts directly — `scripts/scan-miasma-june2026.sh` for the late-June Miasma waves H–I — `binding.gyp` Phantom Gyp detection, the LeoPlatform/RStreams + ImmobiliareLabs package lists, `.github/workflows` injection, `runOn:folderOpen` autorun, and all-event `.claude/settings.json` hook auditing; `scripts/scan-shai-hulud-may2026.sh` for the Shai-Hulud / Miasma / Phantom Gyp / Atomic-Arch waves through June 2026 (npm IOCs A–G plus the Miasma Bun/JS temp-file checks; PyPI Hades waves F–G are covered as documented `pip list` checks in the script comments); and `scripts/scan-g747-may22.sh` for the May 22 IOCs — postinstall worm, Laravel-Lang RCE, Nx Console v18.95.0, TrapDoor zero-width injection — or check manually):
