...
2. **Check for persistence** (run `scripts/scan-miasma-june2026.sh` for the late-June Miasma waves H–I — `binding.gyp` Phantom Gyp detection, the LeoPlatform/RStreams + ImmobiliareLabs package lists, `.github/workflows` injection, `runOn:folderOpen` autorun, and all-event `.claude/settings.json` hook auditing; `scripts/scan-shai-hulud-may2026.sh` for the Shai-Hulud / Miasma / Phantom Gyp / Atomic-Arch waves through June 2026 (npm IOCs A–G plus the Miasma Bun/JS temp-file checks; PyPI Hades waves F–G are covered as documented `pip list` checks in the script comments); and `scripts/scan-g747-may22.sh` for the May 22 IOCs — postinstall worm, Laravel-Lang RCE, Nx Console v18.95.0, TrapDoor zero-width injection — or check manually):
