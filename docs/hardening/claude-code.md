...
- **CVE-2026-24887**: Deeplink-based RCE — the `eagerParseCliFlag` function scanned the entire command-line array for any `--settings=` string without checking whether it was a standalone flag or a value passed to another flag. Combined with the `claude-cli://` deeplink handler, an attacker could embed a malicious `--settings=` payload in a URL that injected a `SessionStart` hook, achieving RCE the moment the victim clicked the link. Fixed in v2.1.118. ([SentinelOne](https://www.sentinelone.com/vulnerability-database/cve-2026-24887/) | [0day.click writeup](https://0day.click/recipe/2026-05-12-cc-rce/))
...
- **Network sandbox SOCKS5 null-byte bypass (no CVE, silently patched v2.1.90)**: Researcher Aonan Guan found that injecting a null byte into a SOCKS5 hostname — e.g. `attacker-host.com\x00.allowed-domain.com` — caused the allowlist filter to approve the connection (it reads the trailing `.allowed-domain.com`) while the OS truncated at the null byte and connected to `attacker-host.com`. Affected v2.0.24 through v2.1.89 (5.5 months, ~130 versions). Silently patched April 1, 2026 with no release note and no CVE. If you used wildcard allowlists (`*.domain.com`) before upgrading to v2.1.90, audit outbound SOCKS-mediated traffic from that period and rotate all reachable credentials. ([SecurityWeek](https://www.securityweek.com/anthropic-silently-patches-claude-code-sandbox-bypass/) | [Aonan Guan](https://oddguan.com/blog/second-time-same-sandbox-anthropic-claude-code-network-allowlist-bypass-data-exfiltration/))
- **TrustFall — MCP auto-approval trust dialog weakened (May 2026)**: Adversa AI found that a malicious repository shipping two small JSON files auto-approves an attacker-controlled MCP server, which starts as an OS process with full user privileges when the developer accepts the trust dialog. The v2.1 dialog removed the "trust folder with MCP disabled" option that earlier versions offered. In CI/CD headless mode (GitHub Actions), no dialog appears — any PR that includes a malicious project config triggers the MCP server automatically. **Mitigation:** Review every `.claude/settings.json` and any `mcpServers` block in cloned repos before accepting the trust prompt. Never accept the trust dialog on a CI runner without auditing project configs. ([Adversa AI — TrustFall](https://adversa.ai/blog/trustfall-coding-agent-security-flaw-rce-claude-cursor-gemini-cli-copilot/))
...
**Security note:** A SOCKS5 null-byte injection bypass (no CVE) affected v2.0.24 through v2.1.89 — wildcard `allowedDomains` entries like `*.npmjs.org` could be bypassed. Silently patched in v2.1.90 (April 1, 2026). Verify you are on v2.1.90 or later before relying on this allowlist for security.
