...
- **v2.1.186** (June 22, 2026): Three improvements — (1) MCP Authentication: added `claude mcp login <name>` and `claude mcp logout <name>` for secure per-server auth without interactive menus; (2) Permission improvements: background subagents now surface permission prompts in the main session instead of auto-denying, with dialogs showing which subagent is requesting; (3) Auto Mode Safety: destructive git commands (`git reset --hard`, `git checkout -- .`, `git clean -fd`, `git stash drop`) and cloud infra-destroy commands (`terraform destroy`, `pulumi destroy`, `cdk destroy`) blocked when user did not explicitly request discarding work (extends and clarifies the v2.1.183 auto-mode restrictions). ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.187** (June 23, 2026): Four fixes — (1) `sandbox.credentials` new setting blocks credential-file patterns (`.env`, `.netrc`, `~/.ssh/*`, `~/.aws/credentials`) from being read inside sandboxed commands, preventing credential leakage through the sandbox; (2) fixed StructuredOutput infinite loop when using `--json-schema` CLI flag or workflow `agent({schema})` — schema-validation mismatch caused unbounded retries instead of surfacing the error; (3) fixed remote MCP tool calls hanging for 5 minutes before timing out when the server drops the connection mid-stream; (4) fixed leaked agent worktree registrations persisting after a session ends; org model restrictions now respected in the model picker UI. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.191** (June 24, 2026): Three fixes — (1) fixed background agents resurrecting after being stopped when TaskStop was called, allowing stopped agents to continue consuming resources and executing tool calls; (2) improved sandbox network permission dialog — hosts approved with "Yes" are now remembered for the rest of the session instead of re-prompting on every connection attempt; (3) improved MCP OAuth retry support with better error handling for headless/CI environments where interactive auth flows are unavailable. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.193** (June 25, 2026): Quality and resilience update — adds `autoMode.classifyAllShell` managed setting to route all Bash/PowerShell commands through the auto-mode safety classifier regardless of whether an explicit allow rule matched; adds a startup notice when MCP servers require authentication (previously silent); adds automatic memory-pressure reaping for idle background shell commands to prevent runaway resource use in long sessions; improves background agent handling and MCP OAuth retry logic. No standalone security CVEs in this release. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.195** (June 26, 2026): Two security fixes — (1) hook matcher bypass: hyphenated tool/MCP identifiers (e.g., `mcp__brave-search`, `code-reviewer`) were accidentally substring-matching instead of exact-matching, which could cause security hooks to fire on the wrong tools or miss the intended tools entirely; (2) external plugin consent bypass: plugins enabled only by project `.claude/settings.json` were not requiring explicit install consent on every loader path, allowing a project-scoped plugin to load without the user's knowledge on certain startup sequences. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
