...
- **CVE-2026-24887**: Command injection via the `find` sub-command — the parser failed to validate command structure, allowing attackers to craft inputs that bypassed the security confirmation mechanism. Fixed in v2.0.72. ([SentinelOne](https://www.sentinelone.com/vulnerability-database/cve-2026-24887/))
...
- **CVE-2026-40068** (May 2026, CVSS 7.7): Trust dialog bypass via git worktree `commondir` spoofing — Claude Code read the `commondir` file verbatim without validating the path, allowing a malicious repository to spoof a previously trusted directory and silently execute `.claude/settings.json` hooks with no user prompt. Affects versions 2.1.63–2.1.83. Fixed in v2.1.84. CWE-20 + CWE-77. Reported by masato_anzai (HackerOne). ([GitHub Advisory GHSA-q5hj-mxqh-vv77](https://github.com/anthropics/claude-code/security/advisories/GHSA-q5hj-mxqh-vv77))
...
- **May 2026 security patches**: Fixed `dangerouslyDisableSandbox` running commands outside the sandbox without a permission prompt (Bash sandbox bypass). Fixed `find -exec` allow rules auto-approving `find -exec/-delete`. Fixed sandbox auto-allow bypassing the dangerous-path safety check for `rm`/`rmdir` targeting `/`, `$HOME`, or other critical paths. Fixed `allowManagedDomainsOnly`/`allowManagedReadPathsOnly` being ignored when a higher-priority managed-settings source lacked a sandbox block. Update to v2.2.x or later. ([Releasebot changelog](https://releasebot.io/updates/anthropic/claude-code))
...
- **SymJack** (May 2026): Adversa AI disclosed an architectural flaw affecting Claude Code, Cursor, Gemini CLI/Antigravity CLI, GitHub Copilot CLI, Grok Build, and OpenAI Codex CLI. A booby-trapped repo places a renamed symlink as the project instruction file (CLAUDE.md, `.cursorrules`); the agent's "file copy" approval shows the symlink name, not its target — the agent follows the symlink and overwrites its own config (`settings.json`, `mcp.json`) with a malicious MCP server entry. On the next restart, the attacker-controlled MCP server runs with full user privileges. No CVE assigned as of May 2026. Mitigated by auditing symlink status of project instruction files before approving any copy operations in untrusted repos. ([Adversa AI](https://adversa.ai/blog/the-approval-prompt-is-lying-to-you-symlink-rce-in-five-ai-coding-agents-claude-code-cursor-antigravity-copilot-grok-build/))
- **v2.1.160** (June 2, 2026): Added a permission prompt before writing to shell startup files (`.zshenv`, `.zlogin`, `.bash_login`, `~/.config/git/`) — these are sourced on every shell start and have been exploited by worm payloads as a silent persistence vector. In `acceptEdits` mode, also prompts before writing build-tool config files that grant code execution (`.npmrc`, `.yarnrc*`, `bunfig.toml`, `.bazelrc`, `.pre-commit-config.yaml`, `.devcontainer/`). ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **Claude Code GitHub Actions `checkWritePermissions` bypass** (June 2026, CVSS v4.0: 7.8): The `anthropics/claude-code-action` GitHub Actions workflow's `checkWritePermissions` function unconditionally trusted any actor ending in `[bot]` regardless of actual permissions. An unauthenticated external attacker could create a GitHub App, use its installation token to open an issue or PR on any public repo using the workflow, and trigger it with attacker-controlled content. Combined with prompt injection, this enables full supply chain attack: exfiltrate secrets, steal OIDC tokens, push malicious code. Patched in Claude Code GitHub Actions v1.0.94 (added `checkHumanActor`, disabled workflow run summaries, scrubbed child-process env vars, added `gh` URL-validation wrapper). Researcher: RyotaK, GMO Flatt Security. ([flatt.tech — Poisoning Claude Code](https://flatt.tech/research/posts/poisoning-claude-code-one-github-issue-to-break-the-supply-chain/))
- **TrustFall** (May 2026): Adversa AI disclosed an architectural flaw in four AI coding agents — Claude Code, Gemini CLI, Cursor CLI, and GitHub Copilot CLI. Project-defined MCP servers execute automatically after a folder trust prompt is accepted; one Enter keypress on a malicious repository causes RCE. In CI headless mode, zero user interaction is required. Anthropic's stated position: "code execution happens only after the user has consented to the project." No CVE assigned. Mitigated by auditing all MCP server definitions in `.claude/mcp.json` and `.claude/settings.json` before trusting any project directory. ([Adversa AI](https://adversa.ai/blog/trustfall-coding-agent-security-flaw-rce-claude-cursor-gemini-cli-copilot/))
- **v2.1.163** (June 4, 2026): Managed settings now support `requiredMinimumVersion` and `requiredMaximumVersion` constraints — enterprise/team admins can pin agents to a version range and block sessions outside it, preventing rollback to known-vulnerable releases. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.166** (June 6, 2026): Cross-session `SendMessage` hardened against session-ID spoofing; glob pattern validation added to prevent path-traversal abuse via wildcard expansion in tool-use allow rules. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.169** (June 8, 2026): Fixed enterprise managed MCP policies (`allowedMcpServers`/`deniedMcpServers`) not being enforced on reconnect, IDE-typed configs, `--mcp-config` servers during the first session after install, and before remote settings loaded — a policy bypass that could allow restricted MCP servers to execute in managed deployments. Also added `--safe-mode` flag (`CLAUDE_CODE_SAFE_MODE` env var) to start Claude Code with all customizations disabled (CLAUDE.md, plugins, skills, hooks, MCP servers) — useful for incident response when you suspect a compromised config. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.175** (June 12, 2026): `enforceAvailableModels` managed setting now constrains the Default model and prevents user/project settings from widening a managed model allowlist — administrators can lock agents to an approved model set and users cannot override it. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.176** (June 12, 2026): Multiple permission and sandbox hardening fixes — (1) closed an `availableModels` enforcement bypass where `ANTHROPIC_DEFAULT_*_MODEL` environment variables could redirect model alias picks to a model outside the managed allowlist; (2) fixed Linux sandbox failing to start when `.claude/settings.json` is a symlink with an absolute target path; (3) fixed `WebFetch(domain:*.example.com)` wildcard domain rules never matching subdomains in allow/deny/ask positions; (4) fixed file permission rules using mid-pattern wildcards (e.g., `Read(secrets-*/config.json)`) being silently rejected at startup instead of being enforced. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **v2.1.178** (June 15, 2026): Three security fixes — (1) improved auto mode: subagent spawns are now evaluated by the classifier before launch, closing a gap where a subagent could request a blocked action without review; (2) fixed MCP server-level `disallowedTools` specifications being silently ignored for subagents; (3) fixed stale cached request configuration causing model requests to continue failing with auth errors after credentials were refreshed outside the session. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
- **CVE-2026-54316** (GHSA-fg94-h982-f3mm, disclosed June 13, 2026, CVSS 4.0: 6.0 Moderate): Out-of-Band Data Exfiltration via Pre-Approved HuggingFace Domain in WebFetch. `huggingface.co` was hardcoded as a pre-approved bare hostname that bypassed all `--allowedTools` restrictions and permission prompts. A prompt injection could direct WebFetch to attacker-controlled paths on HuggingFace (e.g., `/resolve/main/config.json`), then encode and exfiltrate files, env vars, and command output via HuggingFace's download-count side channel. Reporter: hackerone.com/novee. Fixed in v2.1.163 (June 4, 2026). Affects versions >= 0.2.54, < 2.1.163. ([GitHub Advisory GHSA-fg94-h982-f3mm](https://github.com/anthropics/claude-code/security/advisories/GHSA-fg94-h982-f3mm))
- **v2.1.179** (June 16, 2026): Fixed sandbox `denyRead`/`allowRead` glob evaluation over a large directory tree — a correctness bug caused the Bash tool description to balloon to thousands of characters on Linux, making sessions unusable. If your sandbox allow/deny rules cover wide paths (e.g., `~/projects/**`), update to v2.1.179 to restore correct glob evaluation. ([Claude Code changelog](https://code.claude.com/docs/en/changelog))
...
> [2026-06-03 update: Issue #52471 has been **closed as "not planned"** by Anthropic — the team confirmed they do not intend to fix the sandbox's Unix socket IPC blocking for credential managers. The workarounds in [Secret Management](#secret-management) are the long-term path forward. Source: [anthropics/claude-code#52471](https://github.com/anthropics/claude-code/issues/52471)]
