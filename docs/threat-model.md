...
Two days after the Leo Platform wave, the same lineage published **22 malicious versions across four `@immobiliarelabs` Backstage plugins** — `backstage-plugin-gitlab`, `backstage-plugin-gitlab-backend`, `backstage-plugin-ldap-auth`, and `backstage-plugin-ldap-auth-backend` — via the compromised maintainer account `simonecorsi`. The implant reuses the **exact same `binding.gyp`** as the June 3 wave (SHA256 `ef641e95…cca90`), confirming a shared toolkit. The targeting is notable: Backstage is an internal developer-portal framework, and these plugins back **GitLab CI/MR integration and LDAP/AD authentication** — so the blast radius is internal portals, source-control integration, and auth infrastructure. Credentials are staged to GitHub dead-drop repos (the campaign's `liuende501` account held 236 auto-created repos at time of reporting).
