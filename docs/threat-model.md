...
| Unpatched `git.exe` auto-execution zero-day in Cursor IDE (Windows) | Critical | A repository containing a malicious `git.exe` in its project root is auto-executed by Cursor the moment the folder is opened, on a recurring cadence, with no click, prompt, or approval. Reported to Cursor Dec 15, 2025; as of Mindgard's July 14, 2026 public disclosure (seven months later), Mindgard reported no indication that remediation was underway or that affected users had been informed. | [Mindgard — Cursor 0day: When Full Disclosure Becomes the Only Protection Left (July 14, 2026)](https://mindgard.ai/blog/cursor-0day-when-full-disclosure-becomes-the-only-protection-left) |
| "ClaudeBleed Reopened" — untrusted-click bypass in Claude for Chrome extension | High | The extension's content script never checks `event.isTrusted`; any other browser extension with a content script on claude.ai can dispatch a synthetic click to trigger one of Claude's privileged agentic prompts (reading Gmail/Docs/Calendar, modifying Salesforce leads/opportunities) with no real user interaction. Reported May 21, 2026 against v1.0.72; Anthropic shipped v1.0.73-v1.0.80 and marked the internal tracking issue "resolved", but the cited content-script handlers are unchanged and the bug reproduces against v1.0.80 (July 7, 2026). CVSS 7.7 High. | [Manifold Security — ClaudeBleed Reopened](https://www.manifold.security/blog/claude-for-chrome-extension-bypass) |
...
**Why it matters:** both cases end the same way — a researcher reports a real, reproducible flaw, the vulnerable code does not change, and the researcher eventually goes public with a working reproduction and no effective fix — but they got there differently: Cursor's report was reopened and then met with no response or evidence of remediation, while Claude for Chrome's internal tracking issue was marked "resolved" before the July 7 release that still reproduced the bug. Both were still exploitable as of the disclosures on July 14, 2026. For solo developers, that means "check the changelog" is necessary but not sufficient — a shipped version number is not evidence that a reported flaw was fixed.

Source: [Mindgard — Cursor 0day](https://mindgard.ai/blog/cursor-0day-when-full-disclosure-becomes-the-only-protection-left) | [Manifold Security — ClaudeBleed Reopened](https://www.manifold.security/blog/claude-for-chrome-extension-bypass)
