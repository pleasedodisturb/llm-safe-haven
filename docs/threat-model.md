...
| Zero-click prompt injection → sandbox escape → RCE in Cursor ("DuneSlide", CVE-2026-50548 / CVE-2026-50549) | Critical | Cursor's sandbox auto-executes terminal commands without a user approval prompt; a poisoned web search result, a compromised MCP server response, or malicious repository/dependency content ingested by the agent triggers commands that escape the sandbox to full OS-level RCE. No user click required beyond normal agent use. CVSS 9.8 each; fixed in Cursor 3.0 (April 2, 2026), publicly disclosed July 2026. | [Cato Networks — DuneSlide](https://www.catonetworks.com/blog/duneslide-two-critical-rce-vulnerabilities/) (blocked by this session's egress proxy for direct fetch; search-confirmed live via The Hacker News, SecurityWeek, byteiota) |
