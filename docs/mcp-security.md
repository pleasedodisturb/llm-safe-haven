...
|----------|----------|----------|------------|------------------|-----------------|
| `credential-passthrough` | `inlined-secret`, `broad-inheritance`, `sensitive-name-literal`, `high-entropy-literal` | critical / low / high / high | verified | Env values matching a known secret pattern (critical), wildcard/whole-env passthrough tokens (low, advisory), sensitive-key-named literals with an inlined value (high), high-entropy inlined literals (high) | Only sees explicit `env` blocks — a config that implicitly inherits the shell environment by omitting `env` entirely is invisible to a static parser |
