...
| `provenance` | `unverified-offline`, `fetch-failed`, `no-attestation` | info / info / low | unverified / unverified / verified | Whether a resolved npm package has an `dist.attestations` provenance record (only meaningful with `--online`) | Presence-only — never a cryptographic Sigstore verification. Findings say "has/lacks attestation", never claim the package's authenticity was verified |
