...
| [Snyk Agent Security](https://snyk.io/blog/future-of-ai-agent-security-guardrails/) | — | Launched at RSAC 2026 (March). Governs autonomous coding agents from dev to production. Covers Claude Code, Cursor, Windsurf, Gemini CLI, Copilot CLI, and Kiro. Includes `/snyk-fix`, `/snyk-batch-fix` commands, MCP server integration, and an asynchronous hooks-based guardrails engine that replaces rules-based blocking. |
...
| Resource | Stars | Description |
|----------|-------|-------------|
| [NVIDIA/garak](https://github.com/NVIDIA/garak) | ~7K | LLM vulnerability scanner. Probes for hallucination, data leakage, prompt injection, misinformation, toxicity, jailbreaks. ~100 attack vectors, up to 20K prompts per run. AVID integration for community vulnerability sharing. Apache-2.0. **v0.15.0 (May 1, 2026):** adds multi-turn GOAT probe, Agent-breaker probe for testing tool-equipped agents, system-prompt-extraction probe, ModernBERT refusal detector, and NeMo Guardrails server support. |
...
| [StepSecurity: "Megalodon: Mass GitHub Actions Secret Exfiltration"](https://www.stepsecurity.io/blog/megalodon-mass-github-actions-secret-exfiltration-across-5-500-public-repositories) | May 2026 | 5,561 GitHub repos backdoored in 6 hours via automated CI workflow injection. Two payload variants: mass (every push/PR) and targeted (dormant workflow_dispatch backdoors). Credentials bootstrapped via infostealer infections. |
| [Socket.dev: "TrapDoor Crypto Stealer Supply Chain Attack"](https://socket.dev/blog/trapdoor-crypto-stealer-npm-pypi-crates) | May 2026 | 34 packages across npm, PyPI, Crates.io targeting crypto/DeFi/AI developers. First confirmed use of zero-width Unicode in CLAUDE.md/.cursorrules to trick AI coding assistants into credential exfiltration. Full IOC list included. |
| [VentureBeat: "GitHub confirms 3,800 internal repos stolen via poisoned VS Code extension"](https://venturebeat.com/security/github-confirms-3800-repos-stolen-poisoned-vs-code-extension-supply-chain-worm-microsoft-python-sdk) | May 2026 | TeamPCP (UNC6780) compromised GitHub's internal repos via trojanized Nx Console VS Code extension. Extension was live for ~11 minutes on the Marketplace before removal. The trusted namespace was the attack surface. |
| [The Register: "Even Claude agrees: hole in its sandbox was real and dangerous"](https://www.theregister.com/security/2026/05/20/even-claude-agrees-hole-in-its-sandbox-was-real-and-dangerous/5243662) | May 2026 | Claude Code SOCKS5 hostname null-byte injection bypasses sandbox allowlist. Second silent fix in five months; no CVE, no public advisory. Pattern of undisclosed AI security patches analyzed. |
