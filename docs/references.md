...
| [HeadyZhang/agent-audit](https://github.com/HeadyZhang/agent-audit) | ~173 | Static security scanner for LLM agent *code* — the complementary layer to MCP server scanning. 53 rules mapped to all 10 OWASP Agentic Top 10 categories. AST-based dataflow analysis, credential detection, taint tracking, MCP config auditing. Works with LangChain, CrewAI, AutoGen. CI/CD integration via GitHub Actions. 94.6% recall. `pip install agent-audit`. February 2026. |
| [sattyamjjain/agent-audit-kit](https://github.com/sattyamjjain/agent-audit-kit) | ~6 | Security scanner for MCP-connected AI agent pipelines. 210 rules across 67 detectors, 13 agent platforms (Claude Code, Cursor, VS Code Copilot). Full OWASP Agentic 10/10 + MCP Top 10 coverage; EU AI Act/SOC 2/ISO 27001/HIPAA mapping; SARIF output; GitHub Actions integration. Rule bundles cryptographically signed; 48-hour SLA for CVE coverage. `pip install agent-audit-kit`. |
...
| [VentureBeat: "GitHub confirms 3,800 internal repos stolen via poisoned VS Code extension"](https://venturebeat.com/security/github-confirms-3800-repos-stolen-poisoned-vs-code-extension-supply-chain-worm-microsoft-python-sdk) | May 2026 | TeamPCP breached GitHub via poisoned Nx Console VS Code extension (v18.95.0), exfiltrating 3,800 internal repositories. Demonstrates IDE extension as a high-impact developer trust surface attack vector — the same class as the OpenVSX namespace hijacking affecting Cursor and Windsurf. |
| [Microsoft Security Blog: "Mini Shai-Hulud: Compromised @antv npm packages enable CI/CD credential theft"](https://www.microsoft.com/en-us/security/blog/2026/05/20/mini-shai-hulud-compromised-antv-npm-packages-enable-ci-cd-credential-theft/) | May 2026 | Microsoft's official analysis of the AntV wave, including technical IOC details and defense recommendations for CI/CD pipelines. |
